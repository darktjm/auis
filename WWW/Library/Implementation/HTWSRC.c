...
PRIVATE void WSRCParser_put_string (HTStream * context, CONST char* str)
{
    CONST char *p;
    for(p=str; *p; p++)
...
PRIVATE int WSRCParser_abort (HTStream * me, HTError e)
{
    WSRCParser_free(me);
    return EOF;
}
