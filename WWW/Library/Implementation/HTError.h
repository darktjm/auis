...
*//*
   ...
   This module is implemented by HTError.c, and it is a part of the Library of Common
   Code.
...
extern CONST char *HTErrorGetPrefix (void);/*
  ...
  GENERATING AN ERROR MESSAGE (DEFAULT TO STANDARD ERROR)
   ...
   error messages can be handled to the user in a nice  way. That is the reason for
   putting the actual implementation in HTErrMsg.c, that can be overwritten by clients and
   servers apart from Line Mode Browser.
   
   Note: If a stream has been put up (and maybe taken down again) inside the  Library,
