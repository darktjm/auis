...

{
    ...
    if (HTClientHost) {
	if (TRACE) fprintf(TDEST, "Only caching if WWW is run locally.\n");
	return HTBlackHole();
    }
    me = (HTStream*)calloc(sizeof(*me),1);
	...
	if (TRACE) fprintf(TDEST, "HTStream: can't open %s for writing\n",fnam);
	free(fnam);
	free(me);
	return NULL;
    }
    ...
    if (TRACE) fprintf(TDEST, "Cache....... Creating file %s\n", fnam);
