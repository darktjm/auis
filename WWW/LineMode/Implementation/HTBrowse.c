...
{ 
  ...
  loop:
    ...
    case 'W': 
	/* Andrew special tpn */
	if (next_word && other_words) {
	    FILE *fp;
	    char fname[256],*p,*fullname,nfname[256];
	    int offset,idx,ha,ox,i;
	    static int ip = 0;
	    static HTAnchor *tpnanchors[1000];
	    this_word = this_word +2;
	    idx = atoi(next_word);
	    if(ip == 0) ip = idx;
	    offset = idx - ip;
	    sprintf(fname,"/tmp/Web%d.web",idx);
	    for(;*other_words != '\0';other_words++)
		if(*other_words == ' ') break;
	    while (*other_words == ' ') other_words++;
	    /*              printf("now other = %s idx = %d ip = %d\n",other_words,idx,ip);
	     */
	    ox=atoi(other_words) - ip;
	    /*              (*actreq)->output_stream.callback = gotcall;*/
	    if(ox < 0) fullname = this_word;
	    else fullname = getfulladdress(this_word,tpnanchors[ox]);
	    for(i = 0; i < offset; i++){
		/* should make this search more efficient */
		if(strcmp(fullname,HTAnchor_address((HTAnchor*)tpnanchors[i])) == 0){
		    sprintf(nfname,"/tmp/Web%d.web",i + ip);
		    if(access(nfname,R_OK) == -1) continue;
		    break;
		}
	    }
	    if(i < offset) {
		/* document already loaded , create link to existing file */
		tpnanchors[offset] =tpnanchors[i];
		link(nfname,fname);
	    }
	    else{
		if ((fp = fopen(fname, "w" )) == NULL) {
		    ErrMsg("Can't access file",other_words);
		    found = NO;
		    break;
		}
		osi_ExclusiveLockNoBlock(fileno(fp));
		*actreq = Thread_new(YES);
		...
		/*              printf("setting tpna[%d] to %d\n",offset,(*actreq)->anchor); */
	    }
	    printf("\n++%d %s\n",idx, fullname);
	    if(ox >= 0)free(fullname);
