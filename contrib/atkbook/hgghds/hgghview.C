static char *hgghview_rcsid = "$Header: /afs/cs.cmu.edu/project/atk-src-C++/contrib/atkbook/hgghds/RCS/hgghview.C,v 1.2 1994/08/11 03:01:32 rr2b Stab74 $";
...
{
    (this)->lpair::SetDataObject( h);
    this->h = h;
    (this->bv)->SetDataObject( h->b);
}
...
static void ChangeButton(class hgghview  *self, long  param)
{
    ...
    if (message::AskForString(self, 50, "Enter new text for button: ", "<BEEP>", Buf, sizeof(Buf)) >= 0) {
	(self->h)->SetButtonText( Buf);
...
void hgghview::ReceiveInputFocus()
{
    class view *v = (this->lp)->GetNth( 0);
    (v)->WantInputFocus( v);
}
