...
static void DeviceControl(class preview  *self);
static void DumpCharacter(class preview  *self,char  c);
static void drawwig(class preview  *self,char  *s);
static char *getstr(char  *p , char  *temp);
static void drawcirc (class preview  *self,long d);
static long dist(long x1, long y1, long x2, long y2);
static void drawarc(class preview  *self,long dx1, long dy1, long dx2, long dy2);
static void drawellip(class preview  *self,long a, long b);
static void conicarc(class preview  *self,long xp, long yp, long x0, long y0, long x1, long y1, long a, long b);
...
static Boolean SetPosition(class preview  *self)
{
   ...
   if (self->PhysicalX != cvx || self->PhysicalY != cvy)
      {
      	(self)->MoveTo(cvx + self->xoff, cvy + self->yoff);
   	self->PhysicalX = cvx;
   	self->PhysicalY = cvy;
      }
   return(TRUE);
...
static void UseFont(class preview  *self,int  f,int  s)
{
   ...
   if (self->NWMFonts == 0 || p->number != n)
	 ...
	 memmove(p + 1, p,(self->NWMFonts - l) * sizeof(struct preview_fontname));
	 p->number = n;
	 if (f >= 17)
	     p->font = NULL;
	 else {
		...
		fontdesc::ExplodeFontName(fname,namebuf,(long) sizeof(namebuf),&style,&size);
		p->font = fontdesc::Create(namebuf,style,size);
	      }
      }
   if (p->font != NULL) (self)->SetFont(p->font);
}
...
static void DrawThing(class preview  *self)
{
   ...
   switch (com[0])
	...
	/*  Don't use the SetPosition call here,since it won't move off screen */
		  ...
		  (self)->DrawLineTo(self->PhysicalX + self->xoff, self->PhysicalY + self->yoff);
		  self->LogicalX += dx;
		  self->LogicalY += dy;
		  break;
	       }
 ...
 */
	if (s[0] == 'f' && s[1] == 'i' && s[2] == 0) {
	    if (SetPosition(self))
	    (self)->DrawString("fi",0);
	}
    else
	if (s[0] == 'f' && s[1] == 'f' && s[2] == 0) {
	    if (SetPosition(self))
	    (self)->DrawString("ff",0);
	}
    else
	if (s[0] == 'f' && s[1] == 'l' && s[2] == 0) {
	    if (SetPosition(self))
	    (self)->DrawString("fl",0);
	}
    else
	if (s[0] == 'F' && s[1] == 'i' && s[2] == 0) {
	    if (SetPosition(self))
	    (self)->DrawString("ffi",0);
	}
    else
	if (s[0] == 'F' && s[1] == 'l' && s[2] == 0) {
	    if (SetPosition(self))
	    (self)->DrawString("ffl",0);
	}
    else {
	l = 0;
	h = sizeof(trs) / sizeof(trsv[0]);
	while (l < h - 1) {
	    m = (l + h) >> 1;
	    p = &trsv[m];
	    if (s[0] < p->name[0]
	...
	if (s[0] == p->name[0] && s[1] == p->name[1] && p->ch) {
	    class fontdesc *ofont = (self)->GetFont();
	    if (p->font != Normal)
		UseFont(self,p->font, self->cursize);
	    if (SetPosition(self))
		(self)->DrawText((char *)&(p->ch),1,0);
	    if (ofont != (self)->GetFont())
		(self)->SetFont(ofont);
	}
    }
    self->PhysicalX = -1;
...
static void DumpCharacter(class preview  *self,char  c)
 {
       ...
       {
       if (c == '_')
	  (self)->FillRectSize(self->PhysicalX +  self->xoff, self->PhysicalY + 1 +  self->yoff,
		(17 *  self->cursize *  self->DisplayResolution + 1200) / 2400, 1,(self)->BlackPattern());
	else 
	    (self)->DrawText(&c,1L,0);
/* 	  putc(c, winout); */
       }
       self->PhysicalX = -1;
}
   ...
   while ((c =  this->peekc) != EOF)
	 ...
	 if (isdigit(c))
	    ...
	    switch (c)
		  ...
		  case 's': 
			 this->cursize = geti(this);
			UseFont( this, this->curfont,  this->cursize);
			break;

		  case 'f': 
			 this->curfont =  this->DviFonts[geti(this)];
			UseFont( this, this->curfont,  this->cursize);
			break;
		  ...
		  case 'H': 
			 this->LogicalX = geti(this);
			break;

		  case 'V': 
			 this->LogicalY = geti(this);
			break;
	 ...
	 if ( this->peekc == 0)
	     this->peekc = getc(f);
	 if ( this->SizeChanged/* || --(PollCount) == 0 && PollWinin() */)
	    return;
...
static void drawwig(class preview  *self,char  *s)	/* draw wiggly line */
	{
    ...
    (self)->MoveTo (xc[1]+ self->xoff, yc[1]+ self->yoff);
    pxp = pyp = -9999;
	...
	for (j = 0; j < numdots; j++) {/* points within */
	    ...
	    w = w - 0.5;
	    t3 = 0.5 * w * w;
	    xp = (long)(t1 * xc[i + 2] + t2 * xc[i + 1] + t3 * xc[i] + 0.5);
	    yp = (long)(t1 * yc[i + 2] + t2 * yc[i + 1] + t3 * yc[i] + 0.5);
	    if (xp != pxp || yp != pyp) {
		(self)->DrawLineTo (xp+self->xoff, yp+self->yoff);
		pxp = xp;
		pyp = yp;
	    }
	}
...
static char *getstr(char  *p , char  *temp)	/* copy next non-blank string from p to temp, update p */
{
    while (*p == ' ' || *p == '\t' || *p == '\n')
	p++;
    if (*p == '\0') {
...
static long dist(long x1, long y1, long x2, long y2)	/* integer distance from x1,y1 to x2,y2 */
{
    float   dx,
            dy;
    dx = x2 - x1;
    dy = y2 - y1;
    return (long)(sqrt (dx * dx + dy * dy) + 0.5);
}
...
static void drawarc(class preview  *self,long dx1, long dy1, long dx2, long dy2)
{
    ...
    x2 = x0 + dx2;		/* "to" */
    y2 = y0 + dy2;
    r = (preview_coordinate)(sqrt ((float) dx1 * dx1 + (float) dy1 * dy1) + 0.5);
...
static void conicarc(class preview  *self,long xp, long yp, long x0, long y0, long x1, long y1, long a, long b)
{
	...
	if (slope == 0.0 && x0 < xp)
		slope = 3.14159265;
	x0 = (long)(xp + radius * cos(slope) + 0.5);
	y0 = (long)(yp + radius * sin(slope) + 0.5);
	slope = atan2((double)(y1 - yp), (double)(x1 - xp));
	if (slope == 0.0 && x1 < xp)
		slope = 3.14159265;
	x1 = (long)(xp + radius * cos(slope) + 0.5);
	y1 = (long)(yp + radius * sin(slope) + 0.5);
   ...
   we hope it ends here too */
	(self)->MoveTo (((preview_coordinate)xc)+ self->xoff, ((preview_coordinate)-yc)+ self->yoff);
	while ((Q >= 0)
	     || ((Q > -2)
	     && ((Xt > Xc)
	     && (Yt < Yc)
		...
		if (dotcount++ > DX) {
			(self)->DrawLineTo (((preview_coordinate)xc)+ self->xoff, (-(preview_coordinate)yc)+ self->yoff);
			dotcount = 0;
		}
		if (Yc < 0.5) {
			/* reinitialize */
			Xs = Xc = 0;
			Ys = Yc = (long)(sqrt((float)(sqr(xs) + sqr(ys))));
	...
	(self)->DrawLineTo (((preview_coordinate)ox1)+ self->xoff,(-(preview_coordinate)oy1)+ self->yoff);
}
