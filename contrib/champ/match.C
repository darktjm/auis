...
void IterateFlaggedEvents(champ_ifefptr proc, long  rock);
int FlagEventsMatchingDate(struct tm  *date);
int matchdate(struct tm  *date, struct datespec  *spec);
static int matchgregoriandate(struct tm  *date, struct gregoriandatespec  *ds);
void IncrementDate(struct tm  *d);
static int isLeap(int  Year);
static struct EcclData *GetEccl(int  year);
static int matchecclesiasticaldate(struct tm  *date, struct ecclesiasticaldatespec  *ds);
#endif
static int matchhebrewdate(struct tm  *date, struct hebrewdatespec  *hds);
void TranslateTmToHebrew(struct tm  *date, struct hebrewdatespec  *hebdate);
...
static int matchhebrewdate(struct tm  *date, struct hebrewdatespec  *hds)
{
    ...
    if (memcmp(&mytm, date, sizeof(struct tm))) {
