...
ATKdefineRegistry(xbitmap, image, NULL);
 ...
 */
  ...
  if ((type = strrchr(name_and_type, '_')) && !strncmp("bits[]", type + 1, 6))
      *type= '\0';
...
long
xbitmap::Read( FILE  *file, long  id )
            {
...
long
xbitmap::Write( FILE  *file, long  writeID, int  level )
                {
