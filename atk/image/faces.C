...
ATKdefineRegistry(faces, image, NULL);
...
faces::Load( const char  *fullname, FILE  *fp )
  ...
  iw = ih = id = 0;
  *fname = *lname = (char) 0;
  while (fgets((char *)buf, BUFSIZ - 1, f)) {
    if (! strcmp(buf, "\n"))
      break;
  ...
  for (x = 0; x < (this)->RGBSize(); x++)
      (this)->RedPixel( x) = 
	(this)->GreenPixel( x) = 
	(this)->BluePixel( x) = (65536 / (this)->RGBSize()) * x;
  (this)->RGBUsed() = (this)->RGBSize();

  /* read in image data
   */

  linelen = w * (this)->Pixlen();
  lineptr = (this)->Data() + (h * linelen);
  for (y = 0; y < h; y++) {
    lineptr -= linelen;
    dataptr = lineptr;
    for (x = 0; x < w; x++) {
      if ((value = nextInt(f, (this)->Pixlen())) < 0) {
...
long
faces::Write( FILE  *file, long  writeID, int  level )
                {
