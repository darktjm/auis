...
void Calculate(class consoleClass  *self, ViceStatistics	 * current, ViceStatistics	 * previous, struct MonitorStats  * dial, int		   index);
void LogIt (int	     level, char     * str);
void	ReConnect(class consoleClass  *self);
void InitHosts(class consoleClass  *self);
void InitDisplayBuffers();
struct RegionLog * WhichErrorLog(int  machine);
void CheckClock(class consoleClass  *self);
void CheckPrint(class consoleClass  *self);
void LogMarinerFetchInfo();
...
void VenusNovelty(class consoleClass  *self);
...
void WakeUp(class consoleClass  *self ) 
    ...
    im::EnqueueEvent(WakeUp, self, event_SECtoTU(Period));
}
...
void	ReConnect(class consoleClass  *self)
    ...
    memmove(&hostn,hoste->h_addr,sizeof(hostn));
    memmove(&tportn,&porte->s_port,sizeof(tportn));
...
void InitHosts(class consoleClass  *self)
	...
	for (i = 0; i < activeClusters; i++) {
	    ...
	    memset(stat1[i], 0, sizeof(ViceStatistics));
	    memset(stat2[i], 0, sizeof(ViceStatistics));
...
void InitDisplayBuffers()
{
    ...
    memset(&datumdum, 0, sizeof(struct datum));
	...
	memmove(datumptr++, &datumdum, sizeof(struct datum));
    }
    memset(&RemoteLogs[0], 0, MAXCLUSTERS * sizeof(struct RegionLog));
    memset(&RegionLogDatums[0], 0, MAXCLUSTERS * sizeof(struct datum));
...
void ChooseMachines(class consoleClass  *self, char  *machinelist)
	...
	memset(stat1[activeClusters],0, sizeof(ViceStatistics));
	memset(stat1[activeClusters],0, sizeof(ViceStatistics));
