...
#if 0
/* MISSING DEFINITION gifin_close_image() */
...
gif::Load( const char  *fullname, FILE  *fp )
  ...
  (this)->newRGBImage( gifin_img_width, gifin_img_height, (gifin_l_cmap_flag ?
							 gifin_l_pixel_bits :
							 gifin_g_pixel_bits));
  for (x = 0; x < gifin_g_ncolors; x++) {
    (this)->RedPixel( x) = gifin_g_cmap[GIF_RED][x] << 8;
    (this)->GreenPixel( x) = gifin_g_cmap[GIF_GRN][x] << 8;
    (this)->BluePixel( x) = gifin_g_cmap[GIF_BLU][x] << 8;
  }
  (this)->RGBUsed() = gifin_g_ncolors;
  ...
  if (gifin_interlace_flag) {
    scanlen = (this)->Height() * (this)->Pixlen();
    ...
    for (pass = 0; pass < 4; pass++) {
      ...
      scanlen = (this)->Width() * (this)->Pixlen() * interlace_rate[pass];
      pixline = (this)->Data() + (y * (this)->Width() * (this)->Pixlen());
	...
	for (x = 0; x < gifin_img_width; x++) {
	  ...
	  valToMem(pixel, pixptr, (this)->Pixlen());
	  pixptr += (this)->Pixlen();
	}
...
long
gif::Write( FILE  *file, long  writeID, int  level )
                {
...
gif::WriteNative( FILE  *file, const char  *filename )
	...
	*(b + i) = (this)->BluePixel( i) >> 8;
    ...
    if(file) {
	WriteGIF(file, (this)->Data(), (this)->Width(), (this)->Height(), r, g, b, (this)->RGBUsed(), colorstyle);
    }
    else {
	FILE *f;
	if(filename) {
