...
static struct tmenu *CreateMenu(struct menubar  *mb,const char *title,int  prio)
{
	...
	memmove(t2, mb->menus, mb->nmenus*sizeof(struct tmenu *));
	free(mb->menus);
	mb->menus=t2;
    }
...
void mb_RefitMenubar(struct menubar  *mb)
{
    ...
    if(mb->resort) {
	mb->resort=False;
	qsort(mb->menus, mb->nmenus,  sizeof(struct tmenu *), QSORTFUNC(mcomp));
    }
...
#ifdef FD_SET
    FD_ZERO(&fdmask);
    FD_SET(mb->mbi->xfd, &fdmask);
#else
    fdmask=1<<mb->mbi->xfd;
#endif
    activatetime.tv_usec=mb->mbi->prefs->activatetime*1000;
    activatetime.tv_sec=mb->mbi->prefs->activatetime/1000;
    nfds=select(mb->mbi->xfd+1, &fdmask, 0, 0, &activatetime);
    if(nfds>0) return 2;
    else return 0;
}
...
#define TITLESINTERSECT(mb,y,h) (((y)>2*SHADOWWIDTH(mb) && (y)<(mb)->mbi->prefs->menubarheight-2*SHADOWWIDTH(mb)) || ((y)<SHADOWWIDTH(mb) && (y)+(h)>2*SHADOWWIDTH(mb) && (y)+(h)<(mb)->mbi->prefs->menu<barheight-2*SHADOWWIDTH(mb)))
/* */
...
static void Configure(Display  *dpy, struct prefs_s  *p, XColor  *fore, XColor  *back)
{
    ...
    ReallyGetColor(dpy, "White", &p->whiteColor);
    ReallyGetColor(dpy, "Black", &p->blackColor);
    
    p->newshadows =  getdefaultbool(dpy, "UseNewShadows", TRUE);
    ...
    p->activatetime=getdefaultint(dpy,"MenubarCardDelay", 0);
...
struct mbinit *mb_Init(Display  *dpy,XColor  *fore,XColor  *back, menubar_exposefptr expose, long  *exposedata, menubar_freefptr freeitem)
{
    ...
    result->dpy=dpy;
#if SY_OS2
    result->xfd = _impsockhandle(ConnectionNumber(dpy), 0);
#else
    result->xfd = ConnectionNumber(dpy);
#endif
