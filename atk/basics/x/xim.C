...
#include <messitem.H>
 ...
 /* avoiding conflict of method called DisplayString with X macro
 called DisplayString... sigh... -rr2b */
...
static long StandardEventMask = STANDARDEVENTMASK;
...
static boolean initialized_session_mgmt = FALSE;
...
static void  InstallMenus(class xim  *self, class menulist  *menulist);
...
static void freeMLCache(class xim  *self,struct mlcacheNode  *cache);
static void  discardCachedML(struct mlcacheNode  *cache , struct mlcacheNode  **rootP);
static struct mlcacheNode * findCachedML(class xim  *self, class menulist  *ml, struct mlcacheNode  *cache, boolean  *newVersionP);
static void QMenuChoice(struct menubar  *mb,char  *idata,char  *mdata);
static void  updateMenus(class xim  *self, class menulist  *ml);
static void  DumpPolyRect(XPoint  * tempRect );
static void  GetValuesFromEvent(class xim  *im, XEvent  * event,XPoint  * tempRect,long  *retWidth ,long  *retHeight );
...
static void PropLoop();
static boolean retrieveProperty(Display  *dpy, Window  wndw, Atom  prop, boolean del, struct expandstring  *cb, boolean  *pXfree, Atom  *pActualtype);
...
static boolean RequestSelection(class xim * self, struct expandstring  *cutBuff, boolean  *xfree);
...
static void sendToCutBuffer(class xim  *self, int  initialmode, struct expandstring  *cb);
static void  deletefromstring(struct expandstring  *b, char  *bx, long  len);
...
static drop_t find_drop_window(Display  *dpy, Window  topwin, unsigned int x_coord , unsigned int y_coord, Window  *win);
...
#define REALVIEWSELECTION() (im::GetSelectionOwner() && im::GetSelectionOwner()->ATKregistry()!=&xim_ATKregistry_) 
	...
	switch(*pref) {
		...
		if(!havebackslash) { /* this isn't escaped */
			...
			*p='\0';
			...
			p=buf;
			pref=strchr(pref,','); /* skip to the comma ending this item */
...
static int CheckMenuChoice(char  **list,int  count,char  *choice)
{
	...
	else if (translatemenus && strcmp(messitem::Replace(list[i]), choice)==0) return i;
    }
    return -1;    
}
...
static struct cardorder *SetupCardOrder(char  *prefname)
{
    int count,i;
    long curprio=10;
    long priostep=environ::GetProfileInt("PriorityStep",5);
...
boolean xim::SupportsTransient()
{
    return TRUE;
}

boolean xim::SupportsOverride()
{
    return TRUE;
}

boolean xim::SupportsOffscreen()
{
    return TRUE;
}
...
void xim::AddAndrewFontPath(Display  * DisplayPtr /* display for font path */)
        {
	ATKinit;
    ...
    if(!environ::GetProfileSwitch("AddFontPath", TRUE)) return;

    andrewDir = environ::GetProfile("AndrewFontDir");
    if(andrewDir==NULL) andrewDir = environ::Get("ANDREWFONTDIR");
    if(andrewDir==NULL) andrewDir = environ::AndrewDir(ANDREW_FONTS_X);

    if(andrewDir && *andrewDir=='\0') return;
    
    strcpy(fontPath, andrewDir);

    filetype::CanonicalizeFilename(fontPath, fontPath, sizeof(fontPath));
...
static void
SetWMProperties(class xim  *self, boolean  nameChanged , boolean  iconic)
{
    ...
    boolean useProgramNameInTitle = environ::GetProfileSwitch("UseProgramNameInTitle", TRUE);

    if(self->IsOffscreenWindow) return;

    if (!initialized_session_mgmt) {
	/* Need to set WM_COMMAND immediately so the session manager can verify
	 * it is restarting the app properly.
	...
	if (self->doing_session_mgmt) {
	    argv = application::AppGetInitialArgv();
	    if (argv != NULL) {
    ...
    memmove(WMName, name, lengthName);
    WMName[lengthName] = ' ';
    if(c)
	memmove(WMName + lengthName + 1, c, clen);
	...
	*c = '.'; *(c+1) = '.';
    ...
    if(!iconic) {
	self->SetStartIconic(FALSE);
	hints->initial_state = NormalState;
    } else {
	(self)->SetStartIconic( TRUE);
    ...
    XChangeProperty(display, window, XA_WM_CLIENT_MACHINE, XA_STRING,
      ...
      * that provide it (most), and WM_SAVE_YOURSELF for the few environments
      * that offer session management (e.g. COSE Desktop).
      */
     clen = 1;
     data[0] = (unsigned long)xim_WM_DELETE_WINDOW;
     if (self->doing_session_mgmt) {
	 data[1] = (unsigned long)xim_WM_SAVE_YOURSELF;
	 clen = 2;
     }
     XChangeProperty(display, xim2window(self), xim_WM_PROTOCOLS, XA_ATOM, 32, PropModeReplace, (unsigned char *)data, clen);
     ...
     */
     ...
     XChangeProperty(display, xim2window(self), drop_protocol, XA_WINDOW,
		     32, PropModeReplace, (unsigned char *)data, 3);
}
...
static int XErrorsToConsole(Display *DisplayPtr, XErrorEvent *ErrorBlock)
{
    char ErrMsg[120], RequestCode[100], CodeNumber[50];
    ATKXErrors++;
    ...
    XGetErrorText(DisplayPtr, ErrorBlock->error_code, 
		ErrMsg, sizeof(ErrMsg));
    sprintf(CodeNumber, "%d", ErrorBlock->request_code);
    XGetErrorDatabaseText(DisplayPtr, "XRequest", 
...
/* Keybindings are mapped to weird escape sequences.
   This may have been a good idea back in the "WM" days, but we
   have far outgrown it.  Sigh.

   As a hack until the keyboard mechanism is improved, we bind
   function keys with ugly codes like this:

      ESC _ x c

   Where x is either 'n' (normal), 's' (shift), 'c' (control) or
   'm' (meta) and c is some character through the ASCII set.

   For legacy reasons many of the keys don't fit this model.  Rather
   than extend the set, it would be better to rework the mechanism.
*/
struct keybinding {
    KeySym key;
    char *binding;
...
#if !SY_AIXx && !defined(IBM032_ENV) 
		/* IBM keyboards don't have L and R keys */
    XK_L1, "\033E",   	/* esc E */
    XK_L2, "\033F",  	/* esc F */
    XK_L3, "\033G",  	/* esc G */
    XK_L4, "\033H",  	/* esc H */
...
static struct keybinding shift_keybindings[] = {
    XK_Home,	"\033_sL",
    XK_Left,	"\033_sM",
    XK_Up,	"\033_sN",
    XK_Right,	"\033_sO",
...
static struct keybinding ctrl_keybindings[] = {
    XK_Home,	"\033_cL",
    XK_Left,	"\033_cM",
    XK_Up,	"\033_cN",
    XK_Right,	"\033_cO",
...
static struct keybinding meta_keybindings[] = {
    XK_Home,	"\033_mL",
    XK_Left,	"\033_mM",
    XK_Up,	"\033_mN",
    XK_Right,	"\033_mO",
...
static class xcolormap **
ColormapForDisplay( Display  *display )
{
...
static void
InitDefaultColormap( class xim  *self )
  {
  ...
  (self)->SetInheritedColormap( (class colormap**) &(displayListItem->colormap));
  (self)->im::InstallColormap( displayListItem->colormap);
}
...
static Display *SetupDisplay(class xim  *self, char  *host)
{
	...
	Append ":0 if needed */
    xhost = (char *)malloc(strlen(host)+3);
    strcpy(xhost, host);
    if (strchr(xhost, ':') == 0) 
	strcat(xhost, ":0");
...
#if SY_OS2
	    /* Since we are depending on EMX, we must use the EMX version of select.
 * However, the file descriptor returned by IBM's XLib is not compatible...but
 * we can't munge it because the IBM XLib won't like that!  So, we keep track
 * of the EMX fd separate.
 */
	    Xfileno = _impsockhandle(Xfileno, 0);
#endif
    ...
    if(environ::GetProfileSwitch("ErrorsToConsole", TRUE)) xim_EstablishConsole(xhost, im::GetProgramName());
    ...
    xim::AddAndrewFontPath(xim2display(self));
	...
	KeyCode keycode;
	KeySym caps_lock=XK_VoidSymbol, num_lock=XK_VoidSymbol;
	KeySym shift[3], ctrl[2], meta[3];	/* Big assumption: 2 shifts, 1 ctrl, 2 metas */
	shift[0] = shift[1] = XK_VoidSymbol;
	ctrl[0] = meta[0] = meta[1] = XK_VoidSymbol;
	boolean numandcapshack = environ::GetProfileSwitch("NumAndCapsLockHack", TRUE);
	...
	if (shift[0] == XK_VoidSymbol)
	    shift[0] = XK_Shift_L;
	if (shift[1] == XK_VoidSymbol)
	    shift[1] = XK_Shift_R;
	if (ctrl[0] == XK_VoidSymbol)
	    ctrl[0]  = XK_Control_L;
	if (meta[0] == XK_VoidSymbol)
	    meta[0]  = XK_Alt_L;
	if (meta[1] == XK_VoidSymbol)
	    meta[1]  = XK_Alt_R;

	/* Now bind our function keys to produce weird escape sequences. */
	/* Bind normal special keys. */
	for (thisBinding = keybindings;
	     thisBinding < keybindings + sizeof(keybindings) / sizeof(struct keybinding);
	     thisBinding++) {
	    XRebindKeysym(xim2display(self), thisBinding->key, NULL, 0, (unsigned char *)thisBinding->binding, strlen(thisBinding->binding));
	    if(numandcapshack) {
		XRebindKeysym(xim2display(self), thisBinding->key, &caps_lock, 1, (unsigned char *)thisBinding->binding, strlen(thisBinding->binding));
		XRebindKeysym(xim2display(self), thisBinding->key, &num_lock, 1, (unsigned char *)thisBinding->binding, strlen(thisBinding->binding));
	    }
	}
	/* Bind shifted special keys. */
	for (thisBinding = shift_keybindings;
	     thisBinding < shift_keybindings + sizeof(shift_keybindings) / sizeof(struct keybinding);
	     thisBinding++) {
	    XRebindKeysym(xim2display(self), thisBinding->key, shift, 2, (unsigned char *)thisBinding->binding, strlen(thisBinding->binding));
	    if(numandcapshack) { /* repeat the hack...sigh. */
		shift[2] = caps_lock;
		XRebindKeysym(xim2display(self), thisBinding->key, shift, 3, (unsigned char *)thisBinding->binding, strlen(thisBinding->binding));
		shift[2] = num_lock;
		XRebindKeysym(xim2display(self), thisBinding->key, shift, 3, (unsigned char *)thisBinding->binding, strlen(thisBinding->binding));
	    }
	}
	/* Bind control special keys. */
	for (thisBinding = ctrl_keybindings;
	     thisBinding < ctrl_keybindings + sizeof(ctrl_keybindings) / sizeof(struct keybinding);
	     thisBinding++) {
	    XRebindKeysym(xim2display(self), thisBinding->key, ctrl, 1, (unsigned char *)thisBinding->binding, strlen(thisBinding->binding));
	    if(numandcapshack) { /* repeat the hack...sigh. */
		ctrl[1] = caps_lock;
		XRebindKeysym(xim2display(self), thisBinding->key, ctrl, 2, (unsigned char *)thisBinding->binding, strlen(thisBinding->binding));
		ctrl[1] = num_lock;
		XRebindKeysym(xim2display(self), thisBinding->key, ctrl, 2, (unsigned char *)thisBinding->binding, strlen(thisBinding->binding));
	    }
	}
	/* Bind meta special keys. */
	for (thisBinding = meta_keybindings;
	     thisBinding < meta_keybindings + sizeof(meta_keybindings) / sizeof(struct keybinding);
	     thisBinding++) {
	    XRebindKeysym(xim2display(self), thisBinding->key, meta, 2, (unsigned char *)thisBinding->binding, strlen(thisBinding->binding));
	    if(numandcapshack) { /* repeat the hack...sigh. */
		meta[2] = caps_lock;
		XRebindKeysym(xim2display(self), thisBinding->key, meta, 3, (unsigned char *)thisBinding->binding, strlen(thisBinding->binding));
		meta[2] = num_lock;
		XRebindKeysym(xim2display(self), thisBinding->key, meta, 3, (unsigned char *)thisBinding->binding, strlen(thisBinding->binding));
	    }
	}
...
{   
	...
	*top = preferedTop;
	...
	if (TopIncrList[topIncrCount] == -99999  &&  LeftIncrList[leftIncrCount] == -99999) {
	    /* if the user wants it to rubberband do so... */
	    GeometryPosition = FALSE;
...
#ifndef RCH_ENV
	/* This hoses up mwm--the max size doesn't account for
	 * the window borders.  Note PMaxSize is not "or'ed" into the flags.
	 */
...
static void DoTransientGeometry(class xim  *self , boolean  override, class xim  *other, int  *left , int  *top , unsigned int  *width , unsigned int  *height, XSizeHints  **sizehintsp , XSizeHints  **zoomhintsp)
{
   ...
   if (preferedLeft == 0) {
    ...
    if(override) {
	(self)->ConfigureRock()|=overridepos;
	if((self)->ConfigureFunction()) (self)->ConfigureFunction()(self, (self)->ConfigureRock(), (self)->ConfigureCustomRock(), other, left, top, width, height);
	*left+=ox;
	*top+=oy;
	if (*left < 0) *left = 0;
	if (*top < 0) *top = 0;
...
static void SetForegroundBackground(class xim  *self, char  **foregroundColor , char  **backgroundColor, class xcolor  **foreground , class xcolor  **background)
{
	...
	*foreground = (class xcolor *)((class xcolormap*) *(self)->CurrentColormap())->Allocate(*foregroundColor);
	if (*foreground == NULL)
	    *foreground = (class xcolor *)((class xcolormap*) *(self)->CurrentColormap())->Allocate( "black");
    }
    else {
	class xcolormap **xcmap = (class xcolormap**) (self)->CurrentColormap();
	if(xcmap && *xcmap)
	    *foreground = (class xcolor *)(*xcmap)->Allocate( "black");
	...
	*background = (class xcolor *)((class xcolormap*) *(self)->CurrentColormap())->Allocate( *backgroundColor);
	if (*background == NULL)
	    *background = (class xcolor *)((class xcolormap*) *(self)->CurrentColormap())->Allocate("white");
	self->backgroundPixel = (*background)->PixelRef();
    }
    else {
	class xcolormap **xcmap = (class xcolormap**) (self)->CurrentColormap();
	if(xcmap && *xcmap)
	    *background = (class xcolor *)(*xcmap)->Allocate("white");
	else
	    fprintf(stderr, "No Current Colormap.\n");
    }
}
...
static struct menubar *MakeStartupMenu(struct mbinit  *mbi, char  *progname)
{
    ...
    strncat(buf, messitem::Replace(" initializing..."), sizeof(buf)-1-strlen(buf));
    buf[255]='\0';
...
boolean xim::CreateWindow(char  *host)
{
    ...
    this->IsOffscreenWindow = FALSE; /* redundant, but useful for debugging offscreen windows */
    if ((xDisplay=SetupDisplay(this, host)) == NULL) return FALSE;
    
    DoGeometry(this, &left, &top, &sizehints, &zoomhints);
...
static void CheckError(class xim *self) {
    Window w;
    int d;
    XGetInputFocus(xim2display(self), &w, &d);
}
...
/* Menu stuff... */
	...
	void 
static ExplodeMenuString(char  *str, char  *paneStr, long  paneStrLen, long  *panePriority, 
...
static void Observe(class xim  *self, class view  *obj)
{
    if(!menuviewp(self, (class view *)obj)) {
	int viewhash=MENUVIEWHASH(obj);
	struct menuviews *new_c=(struct menuviews *)malloc(sizeof(struct menuviews));
	((class observable *)obj)->AddObserver( (class observable *)self); 
	if(new_c) {
	    new_c->next=self->menuviewslist[viewhash];
	    self->menuviewslist[viewhash]=new_c;
	    new_c->view=(class view *)obj;
	}
    }
}
...
InstallMenus(class xim  *self, class menulist  *menulist)
{
	...
	return;
    menulist->curIM = (class im *) self;
   
    if(self->IsOffscreenWindow) return;
    Observe(self, (class view *)menulist->object);
	...
	pt=messitem::Replace(paneString);	
	if (*pt == '\0') {
	    paneTitle = NULL;
	    panePriority = 0;
	}
	...
	if (item->proc != NULL) { /* Is an addition. */
	    ...
	    char *keys=(self->mshowkeys||self->cshowkeys)?(self)->GetKeyBinding( menulist->object, item->proc, item->functionData):NULL;
		...
		if(!self->menubaron || !self->PopupMenuList || CheckMenuChoice(self->PopupMenuList, self->PopupMenuListCount, pt)>=0) {
		    selectionData->refs++;
		    cmenu_AddSelection(self->cmenu, paneTitle, cmenuspanePriority, messitem::Replace(selectionString), selectionPriority, (long)selectionData, cmenu_CreatePane | cmenu_DisallowDuplicates |((menulist)->ItemIsEnabled( item)?cmenu_Active:0), self->cshowkeys?keys:NULL);
		}
	    }
	    if(self->menubaron && self->menu) {
		if((self->mshowinactive || (menulist)->ItemIsEnabled(item))) mb_SetItemStatus(self->menu, paneTitle, selectionString, (menulist)->ItemIsEnabled( item));
		if(keys && self->mshowkeys) mb_SetKeys(self->menu, paneTitle, selectionString, keys);
	    }
	} else {
	    if(self->menubaron && self->menu && (menulist)->ItemIsEnabled(item)) {
		mb_DeleteSelection( self->menu, paneTitle, messitem::Replace(selectionString));
	    }
	    /* delete unwanted menu item. */
	    if(self->cmenuson && self->cmenu && (menulist)->ItemIsEnabled(item)) cmenu_DeleteSelection(self->cmenu, paneTitle, panePriority, messitem::Replace(selectionString), selectionPriority, cmenu_DeleteEmptyPanes);
	    continue; 
	}
    }
...
lastCacheRegion(struct cacheregion  *list)
	{
	...
	static void
linkCacheRegion(struct cacheregion  *region , struct cacheregion  **listP)
	{
	...
	*listP = region;
	...
	static struct cacheregion *
unlinkCacheRegion(struct cacheregion  *region)
	{
	if (region->selfP != NULL)
		*region->selfP = region->next;
	if (region->next != NULL)
	...
	static void
freeCacheRegions(struct cacheregion  *region)
	{
	while(region != NULL) {
...
newCacheNode(struct mlcacheNode  *prev)
		{
...
/* printf("fmlc: freeing a cache\n"); */
	...
	static void 
discardCachedML(struct mlcacheNode  *cache , struct mlcacheNode  **rootP)
	{
	...
	static struct mlcacheNode *
findCachedML(class xim  *self, class menulist  *ml, struct mlcacheNode  *cache, boolean  *newVersionP)
				{
	struct headerlist *h;
	...
	if (cache->version != ml->menuVersion ||  (class im *) self != cache->ml->curIM)
...
void xim::PostMenus(class menulist  *menulist)
		{
	class im *imself = (class im *)this;
	class xim *self = this;
	
	if (xim2window(this) == None)
		return;

	/* the value of NULL for menulist is special it just says to the im re-post all your own menus */
	if(menulist) {
	    (imself->menus)->ClearChain();
	    (imself->menus)->ChainBeforeML( menulist, (long)menulist);
	}
	...
	if (imself->init != NULL) {
		...
		for (i=0;i<CACHEDREGIONS;i++) {
		    struct cacheregion *cc=(struct cacheregion *)malloc(sizeof(struct cacheregion));
		    memset(cc, 0, sizeof(struct cacheregion));
		    linkCacheRegion(cc,
	    ...
	    menulist = (imself->init)->ModifyMenulist( imself->menus);
...
xim::WhichWS()
    ...
    this->seenexpose=FALSE;
    
    this->Xfileno=(-1);

    this->backgroundPixel=(-1);

    this->MenubarRedrawType=mb_FullRedraw;

    this->xloc=this->yloc=(-1);

    this->initversion=(-1);
    
    this->AtomCache=NULL;
    
    this->MouseFacts=NULL;

    this->cmenu=NULL;
    
    this->mbi=NULL;
    this->menu = this->startupmenu = NULL;
    this->IsOffscreenWindow = FALSE;

    memset(this->menuviewslist, 0, sizeof(this->menuviewslist));
    
    this->mlcache=newCacheNode(NULL);
    this->freeRegions=NULL;
    for (i=0;i<CACHEDREGIONS;i++) {
	struct cacheregion *cc=(struct cacheregion *)malloc(sizeof(struct cacheregion));
	memset(cc, 0, sizeof(struct cacheregion));
	linkCacheRegion(cc,
			&this->freeRegions);  /* CORELEAK? -rr2b */
	this->freeRegions->menus=NULL;
	this->freeRegions->cmenus=NULL;
    }
    this->activeRegions=NULL;
    this->globalCursor = NULL;	/* will get on first use */
    this->globalCursorWindow = 0;  /* will get set during createwindow as needed*/
    this->StandardCursor = NULL;  /* Will be  during update */
    this->EverMapped = FALSE;	/* never been mapped */
    this->CurrentlyMapped = FALSE;
    this->popup_active = NULL;	/* No active popup initially */
    this->popup_parent = NULL;	/* We are not a popup */

    
    this->CardOrder = SetupCardOrder("CardOrder");
    this->PopupCardOrder = SetupCardOrder("PopupCardOrder");
    this->MenubarCardOrder = SetupCardOrder("MenubarCardOrder");
    
    this->PopupMenuList = SetupMenuChoices("PopupMenuList", &this->PopupMenuListCount);
    
    this->menubaron = environ::GetProfileSwitch("Menubar", TRUE);
    this->cmenuson = environ::GetProfileSwitch("PopUpMenus", TRUE);
    
    if((!this->menubaron) && (!this->cmenuson)) this->cmenuson = TRUE;
    ...
    this->mshowkeys = environ::GetProfileSwitch("MenubarShowKeys", this->mshowkeys);

    this->cshowkeys = environ::GetProfileSwitch("PopupsShowKeys", this->cshowkeys);
    
    this->mshowinactive = this->cshowinactive = environ::GetProfileSwitch("MenusShowInactive", TRUE);
    
    this->mshowinactive = environ::GetProfileSwitch("MenubarShowInactive", this->mshowinactive);

    this->cshowinactive = environ::GetProfileSwitch("PopupsShowInactive", this->cshowinactive);
    ...
    last_buttonmask = 0;
    visible_state = -1;	/* not watching visibility yet */
    doing_session_mgmt = FALSE;
    
    THROWONFAILURE( TRUE);
	}
static void SafetyCatch(int sig) {
    int i;
    for (i = 0; i < xWindowCtr; i++) {
	XUngrabPointer(windowList[i].display, CurrentTime);
	XUngrabKeyboard(windowList[i].display, CurrentTime);
    }
}

static int GrabPointer(class xim *self) {

    if(self->pointer_grabbed) return 0;
    if(last_grab) {
	last_grab->pointer_grabbed=FALSE;
	override_redirect_count--;
    }
#ifdef GRABPOINTER
    if(XGrabPointer(xim2display(self), xim2window(self), FALSE, ButtonMotionMask|ButtonPressMask|ButtonReleaseMask, GrabModeAsync, GrabModeAsync, None, None, CurrentTime)!=GrabSuccess) {
	XUnmapWindow(xim2display(self), xim2window(self));
	return -1;
    }
    if(XGrabKeyboard(xim2display(self), xim2window(self), FALSE, GrabModeAsync, GrabModeAsync, CurrentTime)!=GrabSuccess) {
	XUnmapWindow(xim2display(self), xim2window(self));
	XUngrabPointer(xim2display(self), CurrentTime);
	return -1;
    }
#endif
    last_grab=self;
    signal(SIGSEGV, SafetyCatch);
    self->pointer_grabbed=TRUE;
    override_redirect_count++;
    return 0;
}

static void UngrabPointer(class xim *self) {
    if(!self->pointer_grabbed) return;
    self->pointer_grabbed=FALSE;
    if(override_redirect_count) {
	override_redirect_count--;
	last_grab=NULL;
	if(override_redirect_count==0) {
#ifdef GRABPOINTER
	    XUngrabPointer(xim2display(self), CurrentTime);
	    XUngrabKeyboard(xim2display(self), CurrentTime);
#endif
	} else {
	    class xim *foundim=NULL;
	    class im *nextim=(class xim *)self->moreRecentlyUsed;
	    if(self->od_parent && self->od_parent->override_redirect) foundim=self->od_parent;
	    while(nextim) {
		if(nextim!=self && nextim->IsType(self->ATKregistry())) {
		    foundim=(class xim *)nextim;
		    if(foundim->override_redirect) {
			break;
		    } else foundim=NULL;
		}
		nextim=nextim->moreRecentlyUsed;
	    }
	    if(foundim==NULL) {
		fprintf(stderr, "im: Internal WARNING: couldn't find an override redirect window.\n");
#ifdef GRABPOINTER
		XUngrabPointer(xim2display(self), CurrentTime);
		XUngrabKeyboard(xim2display(self), CurrentTime);
#endif
	    } else GrabPointer(foundim);
	}
    }
}
xim::~xim()
{
    ...
    class xcolormap **xcmap = (class xcolormap **) (this)->GetInheritedColormap();

    if(last_grab==this) last_grab=NULL;
    
    if(this->StandardCursor) {
	delete this->StandardCursor;
	this->StandardCursor=NULL;
    }
 
    if(this->override_redirect && this->pointer_grabbed) UngrabPointer(this);
    
    if(!this->IsOffscreenWindow) {
	XSelectInput(dpy, xim2window(this), 0);
    }
    
    if(IOwnSelection && owner && (owner)->GetIM()==(class im *)this) {
	(owner)->RemoveObserver( this);
	(this)->GiveUpSelectionOwnership( owner);
    }
    
    if(this->programname) free(this->programname);
    ...
    if(!this->popup_parent) {
	for(i=0;i<MENUVIEWCACHESIZE;i++) {
	    if(this->menuviewslist[i]) {
		struct menuviews *mv=this->menuviewslist[i];
		while(mv) {
		    struct menuviews *next=mv->next;
		    ((class observable *)mv->view)->RemoveObserver( this);
		    free(mv);
		    mv=next;
		}
	    }
    ...
    */
    if (this->globalCursorWindow) XDestroyWindow(xim2display(this), this->globalCursorWindow);

    xgraphic::FinalizeWindow(xim2display(this), xim2window(this));
    if (this->IsOffscreenWindow)
	XFreePixmap(xim2display(this), xim2window(this));
    else
	XDestroyWindow(xim2display(this), xim2window(this));

    if(this->startupmenu && this->startupmenu!=this->menu) mb_Destroy(this->startupmenu);
    
    if(this->mlcache) {
	freeMLCache(this, this->mlcache);
	this->mlcache=NULL;
    }
    if(this->freeRegions) freeCacheRegions(this->freeRegions);
    if(this->activeRegions) freeCacheRegions(this->activeRegions);

    if(this->mbi) mb_Finalize(this->mbi);
   
    if(this->PopupMenuList) FreeMenuChoices(this->PopupMenuList, this->PopupMenuListCount);
    if(this->CardOrder) FreeCardOrder(this->CardOrder);
    if(this->MenubarCardOrder) FreeCardOrder(this->MenubarCardOrder);
    if(this->PopupCardOrder) FreeCardOrder(this->PopupCardOrder);
    ...
    */
    ...
    for (i = 0; i < xWindowCtr; i++)  {
	if (windowList[i].Xfileno == this->Xfileno)  {
	    windowList[i].count -= 1;
	    /* 
	    ...
	    */
		...
		/* note: for the reason given below this FinalizeDisplay call
		    ...
		    so I do it anyway. */
		xcursor::FinalizeDisplay(windowList[i].display);
	...
	if (cur == this) {
	    prev->popup_active = this->popup_active;
	    /* Tell next elt in chain to stack above prev, if prev isn't the popup_parent. */
	    if (prev != this->popup_parent) {
		XWindowChanges changes;

		changes.sibling = xim2window(prev);
		changes.stack_mode = Above;
		XConfigureWindow(xim2display(this->popup_active), xim2window(this->popup_active), CWSibling | CWStackMode, &changes);
	CheckError(this->popup_active);
	    }
	}

	/* find the next im up from us and tell him to post his own menus */
	(this->popup_parent)->PostMenus( NULL);  /* the value of NULL is special it just says to the im "re-post all your own menus" */

    }

    /* If anyone has us as a parent, they must be unmapped, and
      they must not point to us as a parent */
    for	(        im = (class xim *) imList;
		 im != NULL;
		 im = (class xim *) im->next) {
	if (im->popup_parent == this) {
    ...
    XFlush(xim2display(this));
}


static Region curUpdateRgn = NULL;

void xim::HandleRedraw()
{
    long width;
    long height;
    ...
    if(this->menubaron && this->menu) {
	mb_RedrawMenubar(this->menu, this->MenubarRedrawType);
	this->MenubarRedrawType=mb_FullRedraw;
    }
    this->doRedraw = FALSE;
    this->inRedraw = TRUE;
 ...
 */
	...
	if (this->globalCursorWindow) {
	    if(!optimizeprotocol) {
			/* Unmap so no one will watch us! */
		XUnmapWindow(xim2display(this), this->globalCursorWindow);
			/* Remove connections between window and cursor */
		XUndefineCursor(xim2display(this), this->globalCursorWindow);
	    }
		    /* reposition window for later use */
	    XResizeWindow(xim2display(this), 
			  this->globalCursorWindow, width, height);
	CheckError(this);
	...
	if (this->topLevel != NULL)  {
		...
		RedrawBox.x-=point_X(&(this)->GetDrawable()->physicalOrigin);
		RedrawBox.y-=point_Y(&(this)->GetDrawable()->physicalOrigin);
		if(RedrawBox.y<0) {
	...
	if ( ! this->StandardCursor) {
	    class xcursor  * tempC;
	    tempC = (class xcursor *) cursor::Create(this);
	    (tempC)->SetStandard(Cursor_Arrow);
	    (tempC)->Make( this);
	    XDefineCursor(xim2display(this),xim2window(this),tempC->Xc);
	    this->StandardCursor = tempC;
	}
	(this)->UpdateCursors();
	this->inRedraw = FALSE;
    }
...
DumpPolyRect(XPoint  * tempRect )
	...
	static void 
GetValuesFromEvent(class xim  *im, XEvent  * event,XPoint  * tempRect,long  *retWidth ,long  *retHeight )
				{
...
static void ReConfigurePopups(class xim  *im)
{
    class xim *p=im->popup_active;
    while(p) {
	if((p)->ConfigureFunction()) {
	    int x,y;
	    unsigned int w,h, bw, dr;
	    Window dummy;
	    XGetGeometry(xim2display(p), xim2window(p), &dummy, &x, &y, &w, &h, &bw, &dr);
	    y-=MENUBARHEIGHT(im);
	    (p)->ConfigureFunction()(p, (p)->ConfigureRock(), (p)->ConfigureCustomRock(), im, &x, &y, &w, &h);
	    y+=MENUBARHEIGHT(im);
...
/* timer queuing for mouse buttons */
	...
	static void
ButtonTimerFire(struct mouseStatus  *mfacts, long  now)
		{
...
*/
	static void
StartButtonTimeout(class xim  *xim, unsigned int button, long  x , long  y)
			{
	...
	static void
CancelButtonTimeout(struct mouseStatus  *mfacts)
	{
	if (mfacts->event != NULL) 
		(mfacts->event)->Cancel();
	mfacts->event = NULL;
...
*/
	static void
SendButtonUp(struct mouseStatus  *mfacts, long  x , long  y)
		{
	switch (mfacts->state) {
	case msLeftDown:
...
xim::Hit (enum view_MouseAction  action, long  x , long  y , long  clicks)
{
	...
	return this->bbarv->Hit(action, x, y, clicks);	// Hit the button bar container.
    else
...
class im * xim::DoKey(long  key) {
    xim *self=this;
    /* If we have a popup, IT gets the key event */
    while (self->popup_active) 
	self = self->popup_active;
    /* If we have an override_redirect window AND it has the grab, IT gets the key event
     the last one on the chain with a grab wins. */
    xim *s2=self;
    while(s2->od_child) {
	if(s2->od_child->pointer_grabbed) self=s2->od_child;
	s2=s2->od_child;
    }
    return self->im::DoKey(key);
}
...
#define KEYEXPANSIONSIZE 80
	...
	struct displayListItem *displayListItem = displayList;
	while( displayListItem ) {
	    if(display == displayListItem->display && displayListItem->colormap->ColorProtocolEvent(&event)) {
		displayListItem->colormap->HandleXEvent(&event);
		break;
	    }
	    displayListItem = displayListItem->next;
	}
	/* find the im for the window */
	self = (class xim *) imList;
	while(self) {
	    if(((xim2window(self) == event.xany.window) && xim2display(self) == display )) break;
	    self=(class xim *) self->next;
	}
	if (self == NULL) {
		...
		for (ml = MaskList; ml != NULL;  ml = ml->next)
			if (ml->window == event.xany.window) {
				...
				HandleProperty((XPropertyEvent *)&event);
				break;
			}
		continue;
	}
	...
	if (		event.type != GraphicsExpose 
	    ...
	    (self)->DispatchPendingInteractionEvents();
	...
	switch (event.type)  {
		...
		{
		Time cleartime = ((XSelectionClearEvent *)&event)->time;
		if (IOwnSelection && cleartime>=SelectionStartTime && ((XSelectionClearEvent *)&event)->selection == XA_PRIMARY) {
		    (self)->RequestSelectionOwnership( NULL);
		    IOwnSelection = FALSE;
		    CBCacheValid = FALSE;
		}
		}
		...
		RespondToSelectionRequest((XSelectionRequestEvent *)&event);
		break;
	    case SelectionNotify:
		/* our request for a selection has been granted */
		lastEventTime = ((XSelectionEvent *)&event)->time;
		HandleSelectionNotify((XSelectionEvent *)&event);
		break;
	    case PropertyNotify:
	...
	case ButtonPress:
		...
		self->last_buttonmask = buttonEvent->state;
		mfacts->xim=self;
		...
		mouseX = buttonEvent->x - point_X(&(self)->GetDrawable()->physicalOrigin);
		...
		*/
		...
		case MENUBUTTON:
			...
			if(self->cmenu) {
			    int retVal;
			    struct seldata *sel;
			    retVal = cmenu_Activate(self->cmenu, buttonEvent, (long *) &sel, cmenu_BackgroundPixel, self->backgroundPixel);
	...
	*/
	...
	case ButtonRelease:
		...
		case msRightDownPending:
			/* probably the up for the pending down.
			   fire the PendingButtonEvent */
			ButtonTimerFire(self->MouseFacts, -1);
			/* DROP THRU */
		case msLeftDown:
		case msRightDown:
			SendButtonUp(mfacts, mouseX, mouseY);
	...
	case VisibilityNotify:
	    {
	        /* NOTE:  We normally don't select for visibility events.  The
		 * first visibility call to WantInformation will select these events.
		 */
		XVisibilityEvent *visibilityEvent = (XVisibilityEvent *)&event;
		self->visible_state = visibilityEvent->state;
		self->NotifyObservers(0);
	    }
	    break;
	case ClientMessage:
		...
		if ((Atom)clientEvent->data.l[0] == xim_WM_DELETE_WINDOW) {
		    ...
		    (self)->CallDeleteWindowCallback();
		} else if ((Atom)clientEvent->data.l[0] == xim_WM_SAVE_YOURSELF) {
		    /* Session manager wants to save my state. */
		    HandleSessionManagerEvent(self);
		}
	    }
	    break;
	    }
...
xim::HandleMouse(enum view_MouseAction  action, long  x, long  y, long  newButtonState) {
    ...
    xim *other=NULL;
    while(ochild) {
	if(ochild->pointer_grabbed) other=ochild;
	ochild=ochild->od_child;
    }
    if(other) {
	Window discardWindow;
	int ox, oy;
	XTranslateCoordinates(xim2display(this), xim2window(this), xim2window(other), (int)x, (int)y, &ox, &oy, &discardWindow);
	return other->im::HandleMouse(action, ox, oy, newButtonState);	
    } else return im::HandleMouse(action, x, y, newButtonState);
...
*/
	boolean 
xim::HandleFiles (long  twait , boolean  beCheap)
			{
	ATKinit;
...
#define FD_SETSIZE 32
    ...
    if (anyDelivered)
        im::DeliverSignals();		/* avoid non-race case */

#ifdef LWP
    im::LWPCurrentProcess(&imPid);
    ret = im::IOMGRSelect(FD_SETSIZE, &rmask, &wmask, 0, &PollTime);
#else /* LWP */
    ret = select(FD_SETSIZE, &rmask, &wmask, 0, &PollTime);
#endif /* LWP */
    /* deliver the signals */
    if (anyDelivered) im::DeliverSignals();
...
#ifdef LWP
    im::LWPCurrentProcess(&imPid);
    ret = im::IOMGRSelect(32, &rmask, &wmask, 0, &PollTime);
#else /* LWP */
...
xim::RedrawWindow()
	...
	XGetWindowAttributes(xim2display(this), xim2window(this), &attr);
	if (attr.map_state == IsUnmapped)  {
		...
		if(this->override_redirect) {
		    if(GrabPointer(this)) return;
		}

		if((this)->GetStartIconic()) {
		    this->EverMapped=TRUE;
		    return;
		}
		
		ForceLocUpdate(this);
		if ( !this->EverMapped && !this->popup_parent) {
		    this->EverMapped = TRUE;
		    ...
		    while(!(seenexpose)) {
			ForceLocUpdate(this);
			HandleFiles(event_SECtoTU(10),FALSE);
		    }
		    this->CurrentlyMapped = TRUE;
		} else {
		...
		XClearWindow(xim2display(this), xim2window(this));
		(this)->HandleRedraw();
	}
...
static long *CalculateIncrementList(char  *str, long  finalIncr, long  *listCount)
{
    ...
    *listCount = count;
    ...
    memmove(list, incrList, sizeof(long) * count);
    return list;
}
...
xim::InitializeClass() {
	...
	gData = im::GetGlobalData(); /* load up the pointer to shared, global state */
	translatemenus=environ::GetProfileSwitch("TranslateMenus", TRUE);
	useBackingStore = environ::GetProfileSwitch("UseBackingStore", TRUE);
	MouseHysteresis = environ::GetProfileInt("MouseHysteresis", 50);	/* milliseconds */
	if (MouseHysteresis < 1) MouseHysteresis = 1;
	if (MouseHysteresis > 200) MouseHysteresis = 200;
	LeftIncrement = environ::GetProfileInt("LeftIncrement", 10);	/* pixels */
	TopIncrement = environ::GetProfileInt("TopIncrement", 10);	/* pixels */
	str = environ::GetProfile("LeftIncrementList");
	LeftIncrList = CalculateIncrementList(str, LeftIncrement, &maxLeftCount);
	str = environ::GetProfile("TopIncrementList");
	TopIncrList = CalculateIncrementList(str, TopIncrement, &maxTopCount);
	leftIncrCount = 0;
	topIncrCount = 0;
	CurrLeftIncr = CurrTopIncr = 0;
	SelectionAndCB = environ::GetProfileSwitch("SelectionAndCB", TRUE);
	CachedCutBuffer.string = (char *)malloc(CachedCutBuffer.size = 200);
	CachedCutBuffer.length = CachedCutBuffer.pos = 0;

	iconMaxLength = environ::GetProfileInt ("MaxIconLabelLength", iconMaxLength);
	titleMaxLength = environ::GetProfileInt ("MaxTitleLength", titleMaxLength);

	(void)mb_SetGetDefault((GetDefaultsFunction)mygetdefaults);

	(void)proctable::DefineProc("xim-activate-menubar", (proctable_fptr)xim_ActivateMenubar, &xim_ATKregistry_ , "xim", "Activates the menubar in preparation for key traversal with the arrow keys.");

	optimizeprotocol = environ::GetProfileSwitch("OptimizeProtocol", FALSE);

	paranoidlocupdating = environ::GetProfileSwitch("ParanoidWindowLocationCaching", FALSE);

	StandardEventMask |= (environ::GetProfileInt("mouseupmovement", 0)
...
void xim::ClearCursors(class cursor  *Cc )
{
    class xcursor *C=(class xcursor *)Cc;
...
void xim::PostCursor(struct rectangle  *rec,class cursor  *reqC)
{
    class xcursor *reqCursor=(class xcursor *)reqC;
    struct rectangle vrec;
    class cursor * plainCur = (class cursor *) reqCursor;
    class im * imself = (class im *) this;
    ...
    if (this->IsOffscreenWindow) return;
    if (im::IsPlaying()) return;
   ...
   if(!optimizeprotocol) XUndefineCursor(xim2display(this),reqCursor->Xw);
	...
	if (this->popup_active) {
	    ...
	    XConfigureWindow(xim2display(this), reqCursor->Xw, (CWSibling | CWStackMode), &changes);
	CheckError(this);
	} else 
	    XRaiseWindow(xim2display(this), reqCursor->Xw);
         if (ProcessCursor  || imself->WindowCursor) {
	     if (this->popup_active) {
		 changes.sibling = xim2window(this->popup_active);
		 changes.stack_mode = Below;
		 XConfigureWindow(xim2display(this), this->globalCursorWindow, (CWSibling | CWStackMode), &changes);
	CheckError(this);
	     } else 
...
static void updateGlobalCursors(class xim  *self)
{
	...
	if (cursordebug)
	    if (tmp->posted) printf("updateGlobalCursors(xim): posting an active view cursor as a global cursor\n");
...
void xim::SetTitle(char  *title)
{
    ...
    SetWMProperties(this, FALSE, this->GetStartIconic());
}
...
*/
	static void
SetPNMask(Display  *display, Window  window)
		{
	struct masklist *ml;
	for (ml = MaskList; ml != NULL;  ml = ml->next)
		if (ml->window == window) {
	...
	static void
RemovePNMask(Display  *display, Window  window)
		{
...
static void appendtostring(struct expandstring  *string, char  *buffer, long  length)
	...
	if (string->string == NULL) {
		...
		string->string = (char *)realloc(string->string, 
...
*/
	static struct proplistelt *
PostPropList(Window  window, Atom  prop, enum PropState  state, Time  time)
				{
...
*/
	static struct proplistelt *
DequeueProp(Window  window, Atom  atom)
		{
...
*/
	static void
RequeueProp(struct proplistelt  *elt)
	{
	elt->next = PropList;
	PropList = elt;
}
	...
	static void
CancelProp(struct proplistelt  *elt)
	{
	elt->next = FreeProps;
	FreeProps = elt;
}
...
PropLoop()
{
	...
	while (PropList != NULL) {
		xim::HandleFiles(event_ENDOFTIME, FALSE);
...
*/
	static boolean
retrieveProperty(Display  *dpy, Window  wndw, Atom  prop, boolean del, struct expandstring  *cb, boolean  *pXfree, Atom  *pActualtype)
						{
	int actualformat;
...
Atom *xim::GetAtomCache(Display *dpy) {
    return FindAtomCache(dpy);
}
...
HandleSelectionNotify(XSelectionEvent  *event)
	...
	case prop_SelectTarget:
		...
		if (target == None) {
		    ...
		    if(environ::Get( "ATKDebugSelectionProblems")) {
	...
	case prop_Incoming:
		...
		if (actualtype == ac_INCR) {
			...
			*elt->xfree = FALSE;
			...
			nelt->string->string = (char *)malloc(nelt->string->size);
...
HandleProperty(XPropertyEvent  *event)
	...
	Atom actualtype;
	unsigned long nitems, remainingbytes;
	/* seems like we ought to watch for property notify events on the root window for XA_CUTBUFFER0 and clear the CBCacheValid flag if we see one... -rr2b */
...
*/
static boolean
RequestSelection(class xim * self, struct expandstring  *cutBuff, boolean  *xfree)
			{
	Display *display = xim2display(self);
	Window window = xim2window(self);
	Window selOwner;
...
*/
	static boolean
ProcessRequest(XSelectionRequestEvent  *req)
	{
	Atom targets[7];
	...
	struct proplistelt *elt;
	Atom actualtype;
	long OriginalATKXErrors = ATKXErrors;
	...
	if (req->target == ac_TARGETS) {
		...
		XChangeProperty(req->display, req->requestor, req->property,
			XA_ATOM, 32, PropModeReplace, (unsigned char *)targets, 7);
	}
	else if (req->target == ac_TIMESTAMP) {
		XChangeProperty(req->display, req->requestor, req->property,
			XA_INTEGER, 32, PropModeReplace, 
			(unsigned char *)&SelectionStartTime, 1);
	}
		...
		if(!REALVIEWSELECTION()) {
		    ...
		    FILE *pasteFile;
		    xfree=FALSE;
		    pasteFile = im::vfileopen("w", &cutBuffer);
		    if(pasteFile) {
			(im::GetSelectionOwner())->WriteSelection( pasteFile);
			im::vfileclose(pasteFile, &cutBuffer);
		    }
		...
		if (req->target == ac_LENGTH) {
			...
			if (cutBuffer.length < MAXXFER) {
				...
				XChangeProperty(req->display, req->requestor,
					...
					PropModeReplace, (unsigned char *)cutBuffer.string,
					cutBuffer.length);
				if (xfree)  XFree(cutBuffer.string);
				else   free(cutBuffer.string);
			}
		...
		if (OriginalATKXErrors != ATKXErrors) {
...
*/
	static void
ProcessMultiple(XSelectionRequestEvent  *req)
	{
	long success;
	Atom actualtype;
	int actualformat;
...
*/
	static void
RespondToSelectionRequest(XSelectionRequestEvent  *req)
	{
	...
	if(req->property==None) NotificationEvent.property = None;

	XSendEvent(req->display, req->requestor, 
		FALSE, NoEventMask, (XEvent *)&NotificationEvent);
}
	
FILE *xim::OnlyFromCutBuffer()
{
    FILE *pasteFile=NULL;
    struct expandstring cutBuffer;
    boolean xfree;
    ...
    if(CBCacheValid) {
	pasteFile = (FILE *) im::vfileopen("r", &CachedCutBuffer);
    } else {
	retrieveProperty(xim2display(this), RootWindow(xim2display(this), 0), XA_CUT_BUFFER0, FALSE, &cutBuffer, &xfree, &actualtype);
	pasteFile = (FILE *) im::vfileopen("r", &cutBuffer);
...
FILE *xim::OnlyFromSelection()
{
    ...
    if (IOwnSelection &&
	RequestSelection(this, &cutBuffer, &xfree) &&
	cutBuffer.length != 0)
    ...
    } else if(IOwnSelection) retrieveProperty(xim2display(this),  RootWindow(xim2display(this), 0), XA_CUT_BUFFER0, FALSE, &cutBuffer, &xfree, &actualtype);
...
xim::FromCutBuffer()
	...
	} else retrieveProperty(xim2display(this), 
			RootWindow(xim2display(this), 0), 
			XA_CUT_BUFFER0, FALSE, 
			&cutBuffer, &xfree, &actualtype);
	...
	static void
sendToCutBuffer(class xim  *self, int  initialmode, struct expandstring  *cb)
			{
	Display *xDisplay = xim2display(self);
...
*/
	...
	sendToCutBuffer(this, PropModeReplace, &CachedCutBuffer);
	if (SelectionAndCB) {
	    if(im::GetSelectionOwner() && im::GetSelectionOwner()!=this && im::GetSelectionOwner()->IsAncestor(this)) {
		TotallyGiveUpSelection = FALSE;
	    } else this->RequestSelectionOwnership(this);
	}
}

	void 
xim::RotateCutBuffers(long  count)
		{
 	CBCacheValid = FALSE;
	XRotateBuffers(xim2display(this), -count);
}

	void 
xim::AppendToCutBuffer(FILE  *writeFile)
		{
	im::vfileclose(writeFile, &writeCutBuffer);
	sendToCutBuffer(this, PropModeAppend, &writeCutBuffer);
	if (CBCacheValid)
...
void xim::SetWMFocus()
    ...
    if(this->IsOffscreenWindow) return;
    if ( ! this->CurrentlyMapped)
	return;
    /* And make sure if we move the pointer there that the pointer lands on it! */
    XRaiseWindow(xim2display(this), xim2window(this));
    /* And move the mouse there */
    XWarpPointer(xim2display(this), None, xim2window(this), 0,0,0,0,0,0);
    /* be sure it gets the input focus */
    XSetInputFocus(xim2display(this), xim2window(this), RevertToPointerRoot, lastEventTime);
    
    /* Be sure to admit we don't need the input focus anymore */
    XSetInputFocus(xim2display(this), RootWindow(xim2display(this),xim2screen(this)),RevertToPointerRoot, lastEventTime);
    /* do it now! */
    XFlush(xim2display(this));
}
...
xim::ExposeWindow()
	{
	Display *D = xim2display(this);
	if(!this->EverMapped) {
	    (this)->SetAutoMap( TRUE);
	    (this)->RedrawWindow();
	}
	if ( ! this->CurrentlyMapped) {
		...
		XUnmapWindow(D, xim2window(this));  /* does this help? -wjh */
	}
	if (!this->IsOffscreenWindow) {
	    XMapRaised(D, xim2window(this));
	    if(override_redirect && !pointer_grabbed) GrabPointer(this);
	    ForceLocUpdate(this);
	}
	XFlush(D);
	...
	if (this->popup_parent) {
	    XWindowChanges changes;
	    int mask = 0;
	    class xim *parent = this->popup_parent;

	    while (parent->popup_active) {
		if (parent->popup_active == this) return;
		parent = parent->popup_active;
		changes.sibling = xim2window(parent);
		changes.stack_mode = Above;
		mask = CWSibling | CWStackMode;
...
xim::HideWindow()
	...
	if (this->popup_parent) return;
	/* same with offscreen window */
	if(this->IsOffscreenWindow) return;

	if(this->override_redirect) return;
	
	if(!this->EverMapped) {
	    SetWMProperties(this, FALSE, TRUE);
	    XMapWindow(xim2display(this), xim2window(this));
	    this->EverMapped=TRUE;
	    XFlush(xim2display(this));
	    return;
	...
	cmsg.display = xim2display(this);
	cmsg.type = ClientMessage;
	cmsg.message_type = wmchangestate;
	cmsg.window = xim2window(this);
	cmsg.send_event = TRUE;
	cmsg.format = 32;
	cmsg.data.l[0] = IconicState;

	XSendEvent(xim2display(this), RootWindow(xim2display(this), xim2screen(this)), 
...
void xim::VanishWindow()
{
    ...
    XUnmapWindow(xim2display(this),xim2window(this));

    (this)->SetAutoMap( FALSE);
	...
	if (cur == this) prev->popup_active = this->popup_active;
    } else {
	XUnmapEvent unmapnotify;

	unmapnotify.display = xim2display(this);
	unmapnotify.type = UnmapNotify;
	unmapnotify.window = xim2window(this);
	unmapnotify.event = RootWindow(xim2display(this), xim2screen(this));
	unmapnotify.send_event = TRUE;
	unmapnotify.from_configure = FALSE;
	XSendEvent(xim2display(this), unmapnotify.event, FALSE, 
...
*/
	static void
 deletefromstring(struct expandstring  *b, char  *bx, long  len)
			{
...
Unscribe(struct expandstring  *cb)
	...
	cx = strchr(cb->string+flaglen, ',');
	if (cx == NULL) {
		/* invalid ATK datastream: sigh.  Send as is.  */
		return;
	}
	*cx = '\0';
	istext = ATK::IsTypeByName(cb->string+flaglen, "simpletext");
	*cx = ',';
	if (! istext) {
...
static void freeViewsMLCache(class xim  *self,struct mlcacheNode  **cachep, ATK   *view, struct mlcacheNode **parent)
{
    ...
    if(cache->ml && cache->obj && (cache->obj==view || ((class view *)cache->obj)->IsAncestor( (class view *)view))) {
	if(cache->region) {
	    if(cache->region->menus) {
		mb_Destroy(cache->region->menus);
		cache->region->menus=NULL;
...
void xim::UnlinkNotification(class view  *tree)
{
    (this)->im::UnlinkNotification( tree);
    if(this->mlcache) {
	freeViewsMLCache(this, &this->mlcache, (ATK  *)tree, &this->mlcache);
    }
}

void xim::WantInputFocus(class view *requestor) {
    im::WantInputFocus(requestor);
    if(override_redirect) {
	GrabPointer(this);
    }
}

void xim::ObservedChanged(class observable  *changed, long  value)
{
    ...
    this->RemoveObserver(changed);

    if(changed==this->od_child) {
	this->od_child=NULL;
    }
    if(changed==this->od_parent) {
	this->od_parent=NULL;
    }
    
    if((class view *)changed == owner) {
	(this)->GiveUpSelectionOwnership( owner);
    }
    mv=menuviewp(this, (class view *)changed);
    if(mv) {
	mv2=(*mv)->next;
	free(*mv);
	*mv=mv2;
	if(this->mlcache) {
	    freeViewsMLCache(this, &this->mlcache, (ATK  *)changed, &this->mlcache);
	}
    }
}

struct rectangle *xim::GetLoc(class view  *view, struct rectangle  *rect)
{
    long x,y;
    ...
    if(this->updateloc) LocateWindow(this);

    x+=this->xloc;
    y+=this->yloc;
    rect->left = x;
    rect->top  = y + point_Y(&(this)->GetDrawable()->physicalOrigin);
    return rect;
}
...
#endif
*/
	XResizeWindow(xim2display(this), xim2window(this), width, height);
	CheckError(this);
    }
    XFlush(xim2display(this));
    return TRUE;
...
#endif
*/
    XMoveWindow(xim2display(this), xim2window(this), x, y);
	CheckError(this);
    this->xloc = x;
    this->yloc = y;
    XFlush(xim2display(this));
...
void xim::SetBorderWidth(int  n)
{
    if(this->IsOffscreenWindow) return;
    XSetWindowBorderWidth(xim2display(this), xim2window(this), n);
	CheckError(this);
}

boolean xim::CreateOffscreenWindow(class im  *o, int  width , int  height)
{
    Display *xDisplay;
    Pixmap newPixmap;
    ...
    this->IsOffscreenWindow = TRUE;
    this->EverMapped = FALSE;
    this->CurrentlyMapped = FALSE;
    this->menubaron = FALSE;
    this->xloc=0;
    this->yloc=0;

    xim2display(this) = xDisplay = xim2display(other);
...
xim::GetDroppedFiles()
	{
    char **files = this->dropfiles;

    this->dropfiles = NULL;
    return files;
}

void xim::DropFile(char  *pathname, class cursor  *cursor)
			{
    char *pathnames[3];
    ...
    (this)->DropFiles( pathnames, cursor);
}
static drop_t
find_drop_window(Display  *dpy, Window  topwin, unsigned int x_coord , unsigned int y_coord, Window  *win)
{
...
static void send_drop(Display  *dpy, Window  my_win , Window  dest_win, drop_t  drop_type, unsigned int x_coord , unsigned int y_coord , unsigned int button_state, char  **pathnames   /* NULL terminated list */)
{
    ...
    XChangeProperty(dpy, dest_win, prop, prop_type, 8, PropModeReplace, (unsigned char *)prop_data, len);
    free(prop_data);
...
void xim::DropFiles(char  **pathnames, class cursor  *cursor)
			{
	class xcursor *xcursor = (class xcursor *)cursor;
	Window win;
	Bool done;
	XEvent ev;
	XButtonEvent *bev;
	Display *dpy;
	drop_t drop_type;
	static class cursor *standard_cursor = NULL;

	/* Get the cursor to use, and make sure it is made. */
	if (cursor == NULL) {
	    if (standard_cursor == NULL) {
	        /* Define our standard cursor */
		standard_cursor = cursor::Create(this);
		if (standard_cursor == NULL)
		    return; /* Yikes! */
		(standard_cursor)->SetStandard( Cursor_Gunsight);
	    }
	    xcursor = (class xcursor *)standard_cursor;
	} else {
	    xcursor = (class xcursor *)cursor;
	}
	(xcursor)->Make( this);
	dpy = xim2display(this);

	/* Make the user pick a place to drop the files. */
	if (XGrabPointer(dpy, xim2window(this), TRUE,
	...
	while (!done) {
		...
		if (drop_type != drop_notfound)
		    send_drop(dpy, xim2window(this), win, drop_type,
			      bev->x_root, bev->y_root,
			      bev->state, pathnames);
		done = TRUE;
	    }
	}
	XUngrabPointer(xim2display(this), lastEventTime);
}

boolean xim::RequestSelectionOwnership(class view  *requestor)
{
    Display *display = xim2display(this);
    Window window = xim2window(this);
    Window selOwner;
    ...
    if(!(this)->im::RequestSelectionOwnership( requestor)) return FALSE;
  ...
  //  if(IOwnSelection && im::GetSelectionOwner()==requestor) return TRUE;
...
void xim::GiveUpSelectionOwnership(class view  *requestor)
{
    ...
    (this)->im::GiveUpSelectionOwnership( requestor);
	...
	IOwnSelection=FALSE;
    } else if(/* IOwnSelection && */ 1) {
	this->RequestSelectionOwnership(this);
    }
}
...
void
xim::LoseColormap( class colormap  *cmap )
        {
...
void
xim::InstallColormap( class colormap  *cmapc )
{
    class xcolormap *cmap=(class xcolormap *)cmapc;
    (this)->im::InstallColormap( cmapc);
    if(cmap) {
	XSetWindowColormap(xim2display(this), xim2window(this), cmap->XColorMap);
	if(xinstallcolormaps<0) {
	    xinstallcolormaps=environ::GetProfileSwitch("ForceInstallColormaps", FALSE);
	}
	if(xinstallcolormaps) XInstallColormap(xim2display(this), cmap->XColorMap);
    }
}
...
xim::CreateColormap( )
{
    ...
    Window wid = (IsOffscreenWindow) 
		? RootWindow(dpy, scrn) : xim2window(this);
    Colormap XColorMap = XCreateColormap(dpy, wid, visual, AllocNone);
    return new xcolormap(dpy, XColorMap);
}

class color *
xim::CreateColor( char  *name, unsigned int r , unsigned int g , unsigned int b )
{
    if(name) return new color(name);
    else return new color(r, g, b);
}

void xim::Beep(int volume)
{
    if (volume < 0)
	volume = 0;
    else if (volume > 100)
	volume = 100;
    // X uses -100..100 but we use 0..100
    // Perhaps we should use 0..200?
...
char *xim::WantInformation(char *key)
{
	 ...
	 * Return a static string (overwritten on the next call)
	 * containing characters with the following meaning:
	 *
	 *   s   shift key down (right or left)
	 *   c   control key down
	 *   m   meta (alt) key down
	 *
	 *   These are also returned by hit_all_modifiers:
	 *   l   shift/caps lock key "down"
	 *   n   num lock key "down"
	 *
	 * Thus "sc" means both shift and control were down.
	 * Test it like this:
	 *   if (strchr(xx, 's')) {
	 *      / * shift key is down * /
	 *   }
	 *
	 * NOTE:  the implementation for this is dirty!  We keep the
	 * state of the last button event which may not literally match
	 * the Hit method which is probably asking for information.
	 * This will be fixed when we get new button/key management code.
	 */
	char *p;
	static char modset[8];

	p = modset;
	if (last_buttonmask & ShiftMask)
	    *p++ = 's';
	if (last_buttonmask & ControlMask)
	    *p++ = 'c';
	if (last_buttonmask & Mod1Mask)
	    *p++ = 'm';
	if (key[4] == 'a') {
	    /* hit_all_modifiers also checks lock keys */
	    if (last_buttonmask & LockMask)
		*p++ = 'l';
	    if (last_buttonmask & Mod5Mask)
		*p++ = 'n';
	}
	*p = '\0';
	return modset;
    } else if (strcmp(key, "im_visible") == 0) {
	/* Caller is asking for visibility status of the window.
	 * Return a static string (overwritten on the next call)
	 * containing a single char with the following meaning:
	 *
	 *   v   Window is visible.
	 *   o   Window is fully obscured.
	 *   p   Window is partially obscured.
	 *   i   Window is iconified or unmapped (and therefore fully obscured).
	 *
	 * NOTE:  since most apps don't request this, we add this
	 * to our input mask the first time it is queried.  Thus,
	...
	if (visible_state == -1) {
	    ...
	    if (dpy && w)
		XSelectInput(dpy, w, StandardEventMask|VisibilityChangeMask);
	    visible_state = 0;
	}
	if (!CurrentlyMapped)
	    return "i";
	if (visible_state >= 0 && visible_state <= 2)
	    return vistab[visible_state];
	else
	    return "v";	/* bogus state? */
    }
    /* Assume I am the top level view. */
    return NULL;
  }
