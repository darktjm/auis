...
#ifndef NORCSID
#define NORCSID
static UNUSED const char rcsid[]="$Header: /afs/cs.cmu.edu/project/atk-src-C++/atk/examples/ex17/RCS/hwview.C,v 1.5 1996/12/19 20:24:03 fred Exp $";
#endif
...
void helloworldview::SetDataObject(class dataobject *d)
{
    ...
    this->view=(class view *)ATK::NewObject((hw->dobj)->ViewName());
    this->applayer=(this->view)->GetApplicationLayer();
    (this->view)->SetDataObject(hw->dobj);
...
void helloworldview::FullUpdate(enum view_UpdateType  type,long  left,long  top,long  width,long  height )
{
    ...
    if(hw->blackOnWhite){
	(this)->FillRect(
				&myVisualRect,(this)->WhitePattern());
	/* if on white backvround, draw a rectangle around it */
	(this)->DrawRect(&rec);
    }else{
	(this)->FillRect(
				&myVisualRect,(this)->BlackPattern());
...
{    
	...
	if(hw->blackOnWhite)
	    (this)->DrawRectSize(
					this->x-this->frameX-1,
					this->y-this->frameY-1,
					WIDTH+1,HEIGHT+1);
	(this->applayer)->FullUpdate(view_FullRedraw,0,0,WIDTH,HEIGHT);
    }else
