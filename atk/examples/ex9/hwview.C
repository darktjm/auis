...
#ifndef NORCSID
#define NORCSID
static UNUSED const char rcsid[]="$Header: /afs/cs.cmu.edu/project/atk-src-C++/atk/examples/ex9/RCS/hwview.C,v 1.5 1996/12/19 20:21:38 fred Exp $";
#endif
...
{    
	...
	if(this->x!=this->newX || this->y!=this->newY){
	    static char buf[100];
	    sprintf(buf,"Hello world at (%ld,%ld)",this->newX,this->newY);
...
static struct bind_Description helloworldviewBindings[]={
    ...
    {"helloworldview-relocate", "\022",0, "Hello World,Relocate",0,0, (proctable_fptr)relocate, "Relocate the helloworldview string."},
    NULL
};
