...
static long ParagraphIndent(class textview  *self, class text  *text, long  pos, struct formattinginfo  *info)
{
    ...
    pos = (text)->GetBeginningOfLine( pos);

    myEnv = (self)->GetStyleInformation( &mysv, pos, NULL);

    font = fontdesc::Create(mysv.CurFontFamily,
					    mysv.CurFontAttributes, mysv.CurFontSize);

    widthTable = (font)->WidthTable( (self)->GetDrawable());

    spaceWidth = widthTable[(int) ' '];

    while ((c = (text)->GetChar( pos)) == ' ' || c == '\t') {
	pos++;
	if (c == ' ')
	    width += spaceWidth;
	else
    ...
    (self)->ReleaseStyleInformation( myEnv);
    text::FinalizeStateVector(&mysv);
...
static void LocateCursor(class textview  *self, long  startX, long  spaceShim, long  startPos, short  *widths, long  linePos, long  searchPos, struct formattinginfo  *info)
{
	...
	if (tc == ' ')
	    startX += spaceShim;
	if (tc == '\t')
	    startX = GetNextTabPosition(self, startX - bx, &info->sv, info) + bxm;
        else
	    startX += widths[tc];
	startPos++;
    }
...
static void LocateHit(class textview  *self, long  startX, long  spaceShim, long  startPos, short  *widths, long  linePos, long  searchX, struct formattinginfo  *info)
{
	...
	if (tc == ' ')
	    endX += spaceShim;
	if (tc == '\t')
	    endX = GetNextTabPosition(self, endX-bx, &info->sv, info) + bxm;
        else
	    endX += widths[tc];
...
static void AllocateLineItem(class textview  *self, class text  *text, long  pos, struct formattinginfo  *info)
{
    ...
    fontInfo = (tf)->FontSummary( (self)->GetDrawable());
    ...
    if (tsv->CurView == NULL) {
        ...
        tlp->ti_fontWidths = (tf)->WidthTable(
          (self)->GetDrawable());
	...
	if (tlp->vi_height > info->lineBelow) {
	    ...
	    (view)->GetOrigin( tlp->vi_width, tlp->vi_height, &xoffset, &yoffset);
...
static void ComputeStyleItem(class textview  *self, long  startPos , long  endPos, long  *pixelAddr , long  *charAddr, struct formattinginfo  *info)
{
	...
	/* Try to fill it in */
	exposeStylesFont =
          fontdesc::Create("andysans", fontdesc_Plain, 10);
	exposeStylesWidths =
          (exposeStylesFont)->WidthTable(
	    (self)->GetDrawable());
    }
	...
	if (endPos == 0)
	    ...
	    startEnv = (self)->GetStyleInformation( NULL, endPos - 1, NULL);
	else {
	    startEnv = (self)->GetStyleInformation( NULL, endPos, NULL);
		...
		if ((startEnv)->Eval() < i)
                    break;  /* Gone too far */
		startEnv = (class environment *)
                  startEnv->parent;
	    }
	}
    }
    else {
	startEnv = (self)->GetStyleInformation( NULL, startPos, NULL);
    }

    endEnv = (self)->GetStyleInformation( NULL, endPos, NULL);

    tp = &info->lineBuffer[info->lineBP];
    cparent = (class environment *)(startEnv)->GetCommonParent( endEnv);
    newChars = 0;
    newPixels = 0;
    ...
    *charAddr = newChars;
    ...
    (self)->ReleaseStyleInformation( endEnv);
    (self)->ReleaseStyleInformation( startEnv);
}
...
#define GETDCHAR(buf) ((self->password)?'*':(*buf))
    if (self->predrawn && (self->predrawn)->GetLength() > 0 &&
      (self->predrawn)->GetPos() == (currentLine)->GetPos())
	markPos = (self->predrawn)->GetPos() + (self->predrawn)->GetLength();

    bufEnd = (text)->GetLength();		/* The ending point */
    initPos = pos = (currentLine)->GetPos();	/* Initialize it */
    if (bufEnd > pos)
        dotPos = (self->dot)->GetPos();
    oncePerLine = TRUE;
    passes = 0;
    info->lineBP = 0;
    lastBP = 0;
	...
	text::FinalizeStateVector(tsv);
	myEnv = (self)->GetStyleInformation( tsv, pos, &nChars);

        color = tsv->CurColor;
        bgcolor = tsv->CurBGColor;
        
	envLength = (myEnv)->GetLength();

        /* The hidden flag handling probably does not work. */

	if ((tsv->SpecialFlags & style_Hidden) && envLength > 0) {
	    pos += envLength - (myEnv->Eval() - pos);
	    info->lineLength += envLength - (myEnv->Eval() - pos);
	    continue;
	}

	/* Now we have to generate the proper font structure in sv */

	tsv->CurCachedFont = fontdesc::Create(tsv->CurFontFamily,
          tsv->CurFontAttributes, tsv->CurFontSize);

	if (myEnv->type == environment_View) {
	    class viewref *vr = myEnv->data.viewref;
	    if ((tsv->CurView = (class view *)
              dictionary::LookUp(self, (char *) vr)) == NULL) {
		if ((tsv->CurView = (class view *) matte::Create(vr,
                  (class view *)self)) != NULL) {
		    dictionary::Insert(self, (char *) vr,
                      (char *) tsv->CurView);
		    (vr)->AddObserver( self);
                    (vr->dataObject)->AddObserver( text);
		    if (vr == self->currentViewreference)
                      (tsv->CurView)->WantInputFocus( tsv->CurView);
		} else
		    dictionary::Insert(self, (char *) vr,
                      (char *) textview_UNKNOWNVIEW);
	    } else
                if (tsv->CurView == (class view *) textview_UNKNOWNVIEW)
                    tsv->CurView = NULL;
	}
	...
	info->myWidths = (tsv->CurCachedFont)->WidthTable(
	    (self)->GetDrawable());
	...
	if (FounddotPos) {
	    int ipos; 
	    class fontdesc *cfont = info->clp->ti_font;
	...
	for (i = 0; ; i++) {
		...
		if (noWrap || (localRubber <= 0 &&
			...
			do {
			    ...
			    tc = GETDCHAR(buf), buf++, bufLen--, pos++;
			    if(tc != ' ') count++;
			    fullWidth += info->myWidths[(int) tc]; /* NB:  assume current font for speed */
			} while (tc != '\n' && tc != '\r');
		...
		info->lineIP = lastIP;
		*lastBP++ = '\0';
		/* if we saw a tab, check if it is in the part of the line we kept. -rr2b */
		if(info->sawTab && strchr((char *)info->lineBuffer, '\t')==NULL) info->sawTab=FALSE;
		...
		if(info->lineIP>1) {
		    ...
		    *tp++ = 0xff;
		} else localWidth += info->clp->vi_width;
	    } else if (tc == ' ') {
		localRubber++;
		lastWidth = localWidth;
		...
		*tp++ = tc;
		...
		break;
		       ...
		       ! (tsv->CurCachedFont)->CharValid( tc)) {
		if (i == 0) {
		    long width;
...
static void DrawBackground(class textview  *self, struct lineitem  *tt, long  bx , long  by, long  width) {
    if(tt->ti_bgcolor) {
        struct FontSummary *fontInfo;
        long above, below;
        ...
        fontInfo = (tt->ti_font)->FontSummary(
                                              (self)->GetDrawable());
        ...
        self->SetForegroundColor(tt->ti_bgcolor, 0, 0, 0);
        self->FillRectSize(bx, by-above, width, below+above, NULL);
        self->SetForegroundColor(oldcolor, r, g, b);
    }
}
...
static void DrawBar(class textview  *self, struct lineitem  *tt, long  bx , long  by, long  width)
{
    ...
    if (tt->ti_styleFlags & style_StrikeThrough || (self->LineThruFormatNotes && (tt->ti_styleFlags & style_PassThru))) {
        (self)->MoveTo( bx, by - above / 3);
        (self)->DrawLineTo( bx + width, by - above / 3);
    }
	...
	(self)->SetLineDash( "\001\001", 0, graphic_LineDoubleDash);
        ...
        (self)->DrawLineTo( bx + width, by + below);
        (self)->DrawLineTo( bx - 1, by + below);
        (self)->DrawLineTo( bx - 1, by - above + 1);
	(self)->SetLineDash( oldpattern, oldoffset, oldtype);
	if (oldpattern) free(oldpattern);
    }

    if (tt->ti_styleFlags & style_OverBar) {
        (self)->MoveTo( bx, by - above);
        (self)->DrawLineTo( bx + width, by - above);
    }
}
...
static void DrawChangeBar(class textview  *self,struct formattinginfo  *info,long  by)
{
    ...
    if (x > 5) {
	(self)->MoveTo( x - 5, by - info->lineAbove);
	(self)->DrawLineTo( x - 5, by + info->textBelow);
    }
}
...
/*
 * This is the default line number drawing callback.
 * It could serve as a trivial example for a real callback.
 *
 * The "line_mark" is a mark surrounding the current line of
 * text that is being numbered.  Do not save a reference to
 * this mark because it may be destroyed at any time.  Instead,
 * make a copy of the mark with text_CreatMark.
 *
 * The "area" is the rectangle allocated to the line number.
 * Normally a number is drawn right justified using the bottom
 * of the area as a baseline.
 */
static void draw_linenum(textview *self, mark *line_mark, struct rectangle *area, void *rock)
{
    ...
    if (fontID == NULL) {
	fontname = environ::GetProfile("LineNumbersFont");
	if (fontname == NULL)
	    fontname = "AndyType8f";
	if (fontdesc::ExplodeFontName(fontname, ffamily, sizeof(ffamily), &fstyle, &fsize)) {
	    fontID = fontdesc::Create(ffamily, fstyle, fsize);
	    self->para_font = fontID;
	} else {
	    /* Default to the textview's default. */
	    self->para_font = self->GetFont();
	}
    }
    if (fontID)
	self->SetFont(fontID);
...
long textview::LineRedraw(enum textview_LineRedrawType  type, class mark  *currentLine, long  x , long  y, long  xSize , long  ySize, long  search, boolean  *cont, long  *txheight, struct formattinginfo  *info, spotcolor **spots)
{
    ...
    if (type == textview_FullLineRedraw &&
      (this->predrawn)->GetModified() &&
      ! (this->prepredrawn)->GetModified()) {
	(this->predrawn)->GetLength() = (this->prepredrawn)->GetLength();
	this->predrawn->modified = 0;
    }

    GenerateLineItems(this, text, currentLine, info);
    if (type == textview_FullLineRedraw && info->predrawnEnd &&
	this->predrawnY == y + info->lineAbove && this->predrawnX == x && 
	FastJustify(info) && ! (this->predrawn)->GetModified() && 
	this->csxPos > 0 /* Indicates if we are in a full redraw */ )
    ...
    if (! info->endOfPara && (text)->GetChar( info->lineLength + pos - 1) != '\r') {
	long endPos;

	for (endPos = info->lineLength + pos;
          endPos < (text)->GetLength() &&
          (text)->GetChar( endPos) == ' '; endPos++)
            ;
	...
	if (SkipPredrawn != 0 && info->endOfPara && info->lineBelow != info->below) {
	    rectangle_SetRectSize(&clearRect, 0, info->cursorY + info->lineAbove + info->lineBelow, SkipPredrawn + para_width, info->below - info->lineBelow);
	    (this)->FillRect( &clearRect, pat);
	}
	isBlack = FALSE;
    }
	...
	if (info->lineAbove + info->below <= ySize) {
	    ...
	    bx = (hasApplicationLayer) ? this->bx : this->ebx;
	    by = (hasApplicationLayer) ? this->by : this->eby;

	    /* Save the old color */
	    GetForegroundColor(&oldcolor,&cr,&cb,&cg);
	    if (oldcolor != NULL) {
		strncpy(oldcbuf, oldcolor,64);
		oldcbuf[63] = '\0';
		oldcolor = oldcbuf;
	    }
	    rectangle_SetRectSize(&clearRect, 3, info->cursorY, para_width-6,
				  info->lineAbove+5);
	    /* Clip the clear rect so we don't walk over the end bevels. */
	    if (three_d_display) {
		if (clearRect.top < by+2)
		    clearRect.top = by+2;
		if (clearRect.top+clearRect.height > (GetLogicalHeight()-by-by-4))
		    clearRect.height = GetLogicalHeight()-by-clearRect.top-2;
	    }
	    rectangle_SetRectSize(&drawRect, 5, info->cursorY+1, para_width-8,
				  info->lineAbove-1);
	    /* Clear the rectangle. */
	    pat = WhitePattern();
	    if (three_d_display && para_bgcolor) {
		SetForegroundColor(para_bgcolor, 0, 0, 0);
		pat = BlackPattern();
	    }
		...
		SetForegroundColor(oldcolor,cr,cb,cg);
	    if (para_draw_callback)
		(*para_draw_callback)(this, currentLine, &drawRect, para_draw_rock);
	    else
		draw_linenum(this, currentLine, &drawRect, 0);
	    /* Restore the clip rectangle. */
	    SetClippingRect(&oldclipRect);
	    /* Assume the draw callback screwed up the font. */
	    fontID = GetFont();
	    SetFont(fontID);
	...
	if (info->sawTab && i <= leftmost) {
	    (this)->SetSpaceShim( 0); /* No justification left of tab */
	    currentBump =0;
	} else {
	    (this)->SetSpaceShim( info->spaceBump);
	...
	if (tt->type == li_View) {
	    if(tt->vi_view && tt->vi_view==info->searchinset) {
		long xoffset, yoffset;
	...
	/*	if (tt->vi_width > xSize)
		...
		rectangle_SetRectSize(&info->insetrect,
				      bx - xoffset, by - yoffset, tt->vi_width, tt->vi_height);
	    }	
	    if (type == textview_FullLineRedraw) {
		if (bx >= SkipPredrawn || (SkipPredrawn == 0 && bx < 0 && tt->vi_width > -bx)) {
		...
		if (search >= tt->docPos && search < tt->docPos + 1)
            ...
            if (sPtr[0] != '\0' && (type==textview_FullLineRedraw || type==textview_PartialLineRedraw)) {
                /* Only change the color/font if this line item has any chars
                 * in it.  If it is an empty string it doesn't matter if we don't
                 * switch colors/fonts which generates less drawing requests.
                 */
                if (this->showColorStyles) {
                    if(!colordefined && (tt->ti_color || tt->ti_bgcolor || tt->ti_color!=color)){
 ...
 /* foreground  already set*/
                    ...
                    spotcolor *res=NULL;
                    if(info->lineAbove +
                       info->textBelow <= ySize && spots && (tt->ti_color || tt->ti_bgcolor)) {
                        res=new spotcolor;
                        if(tt->ti_color) res->color=atom::Intern(tt->ti_color)->Name();
                        else if(rcolor) res->color=rcolor;
                        else res->color= tempcolor(c1,c2,c3);

                        if(tt->ti_bgcolor) res->bgcolor=atom::Intern(tt->ti_bgcolor)->Name();
                        else if(rbgcolor) res->bgcolor=rbgcolor;
                        else res->bgcolor= tempcolor(bc1,bc2,bc3);
                        struct FontSummary *fontInfo;
                        long above, below;
                        long width= StringWidth(tt->ti_fontWidths, sPtr) + tt->ti_rSpaces * currentBump;
                        fontInfo = (tt->ti_font)-> FontSummary((this)-> GetDrawable());

                        below = fontInfo->maxBelow;
                        above = fontInfo->maxHeight - below;

                        res->top=by-above-1-info->cursorY;
                        res->height=below+above+1;
                        res->left=bx;
                        res->width=width;
                        res->next=(*spots);
                        *spots=res;
                    ...
                    if(color != tt->ti_color) {
                        ...
                        if(color == NULL){
                            ...
                            (this)->SetForegroundColor(rcolor,c1,c2,c3);
                        }
                        else {
                            if(res) SetForegroundColor(&res->color);
                            else {
                                if(spots && (type==textview_PartialLineRedraw || textview_FullLineRedraw)) printf("grn, this shouldn't happen...\n");
                                (this)->SetForegroundColor(color,0,0,0);
                            }
                        }
                    }
                }
		...
		if (type == textview_FullLineRedraw) {
		    /* Drawing a line */
		    if (bx >= SkipPredrawn || (SkipPredrawn == 0 && bx < 0 && StringWidth(tt->ti_fontWidths, sPtr) > -bx)) {
			if (tt->type == li_Expose) {
			    ...
			    (this)->DrawString((char *)
                            ...
                            long sw;
                            if(tt->ti_hasTab && i<info->lineIP) {
                                sw=info->lineItems[i+1].xPos - tt->xPos;
                            } else sw=StringWidth(tt->ti_fontWidths, sPtr) +
                            ...
                            if (tt->ti_styleFlags &
                              (style_Underline | style_StrikeThrough |
                               style_OverBar | style_DottedBox)) {
                                DrawBar(this, tt, bx, by, sw);
			    }
			    if((tt->ti_styleFlags & style_PassThru) && this->LineThruFormatNotes) {
				DrawBar(this, tt, bx, by, sw);
			    }

			    if (tt->ti_styleFlags & style_ChangeBar)
				DrawChangeBar(this, info, by);
			}
		    }
		} else if (type == textview_GetPosition) {
		    /* Looking for a cursor hit */
		    if (tt->type == li_Plain)
			LocateHit(this, bx, currentBump,
                    ...
                    if (tt->type == li_Plain)
                        LocateCursor(this, bx, currentBump,
                          tt->docPos, tt->ti_fontWidths,
                          tt->ti_lineBufPos, search, info);
	...
	(currentLine)->SetLength( info->lineLength);
	currentLine->includeEnding = TRUE;
    } else {
 	(currentLine)->SetLength( info->markLength);
	...
	if (FastJustify(info) && info->itemAtDot->type != li_View) {
	    ...
	    if(chr==EOF) chr=' ';
	    (info->itemAtDot->ti_font)->CharSummary(
              (this)->GetDrawable(), (unsigned char)chr, &cinfo);
	    ...
	    if (ep < cp)
                ep = cp;
	    (this->predrawn)->SetPos( cp);
	    (this->predrawn)->SetLength( ep - cp);
	    this->predrawn->modified = 0;
	    ...
	    if (ep < cp)
                ep = cp;
	    (this->prepredrawn)->SetPos( cp);
	    (this->prepredrawn)->SetLength( ep - cp);
	    this->prepredrawn->modified = 0;
	    this->predrawnY = y + info->lineAbove;
	    this->predrawnX = x;
	} else {
	    /* The mark has been moved, thus this info is useless. */
	    this->prepredrawn->modified = 1;
	    this->predrawn->modified = 1;
	}
    }
    if(color != NULL){
...
void textview::ViewMove(struct linedesc  *lineStructure, long  movement)
{
    class environment *curenv;
    class view *CurView;
    long end, i, elen;
    struct rectangle enclosingRect;
    class mark *currentLine;

    currentLine = lineStructure->data;
    i =  (currentLine)->GetPos();
    end = i + lineStructure->nChars;

    while (i < end) {
	curenv = (this)->GetStyleInformation( NULL, i, &elen);

	if (elen + i > end)
	    elen = end - i;
	if (curenv->type == environment_View &&
	    ((CurView = (class view *) 
	      dictionary::LookUp(this,(char *) (curenv->data.viewref))) != NULL)) {
	    (this)->RetractViewCursors( CurView);
	...
	(this)->ReleaseStyleInformation( curenv);
    }
}
