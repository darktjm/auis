...
#ifndef NORCSID
#define NORCSID
static char rcsid[]="$Header: /afs/cs.cmu.edu/project/atk-src-C++/atk/raster/lib/RCS/xwdio.C,v 1.4 1994/11/30 20:42:06 rr2b Stab74 $";
#endif
...
long xwdio::ReadRow(FILE  *file		, unsigned char *row	, long  nbytes   )
...
void xwdio::WriteRow(FILE  *file, unsigned char *row, long  nbytes)
...
long xwdio::ReadImage(FILE  *file		, class pixelimage  *pix	)

{
    ...
    rowcount = hdr.window_width;
    (pix)->Resize( (rowsize * 8), hdr.pixmap_height);
    (pix)->Clear();
    row = (pix)->GetBitsPtr();
	...
	row += ((rowsize + 1) / 2) * 2, rownum++) {	
	    xwdio::ReadRow(file, src, rowsize);
	    ...
	    if (rownum & 0x01)
		RightToLeft(curr, next, rowcount);
	    else
		LeftToRight(curr, next, rowcount);
	...
	{	
	     ...
	     */
	    xwdio::ReadRow(file, row, rowsize);
...

{
	...
	xwdio::WriteRow(file, (unsigned char *)buf, bytes_per_row);
    }
}
