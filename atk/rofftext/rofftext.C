...
static int g(class rofftext  *self,Trickle  t)
{
    ...
    switch (cur->type) {
	    ...
	    /* modified to keep track of line numbers */
            if (c == '\n') {
		if (strcmp((self)->GetTypeName(), "mmtext") == 0) {
		    class mmtext *doc = (class mmtext *) self;
		    (doc)->SetLinePos( cur->LineNumber, self->pos);
		}
		else if (strcmp((self)->GetTypeName(), "mantext") == 0) {
		    class mantext *doc = (class mantext *) self;
		    (doc)->SetLinePos( cur->LineNumber, self->pos);
		}
                cur->LineNumber++;
	    }
    ...
    if ((c == EOF) && (cur->prev != NULL)) {
        ...
        if (cur->type == trickle_File)
            ...
            if (cur->pop) {
                ...
                self->CurrentContext = (IC)(self->ArgStack)->Pop();
            }
            free(cur->buf);
        }
        free(cur);
...
/* have to worry about multiple ung's and EOF and NULCHAR
	just tpush another trickle for the single char !    -wjh */
void ung(class rofftext  *self,int  c,Trickle  t)
{
    char buf[3];
    switch (c) {
    ...
    tpush(self,t,NULL,NULL,buf,FALSE,0,NULL);
}
#if 0
{
    register struct _trickle *cur = t->t;
...
static void special(class rofftext  *self,Trickle  t)
{
    ...
    temp[1] = g(self,t);
    temp[2] = '\0';
    result = (self->SpecialChars)->Lookup(temp);
    if (result)
        tpush(self,t,NULL,NULL,result,FALSE,0,NULL);
    else {
        temp[0] = 'X';
        temp[1] = '\0';
...
DEBUG(1, (stderr, "value of .$ is %s\n", temp));
    ...
    r = (Reg)(self->Registers)->Lookup(temp);
    if (r == NULL) {
...
void putregister(class rofftext  *self,char  *name,int  value,enum RegFmt  fmt,int  inc,boolean  relative)
{
    ...
    if (r==NULL) {
        ...
        r->format = fmt;
        r->autoinc = inc;
        (self->Registers)->Store(name,(char *)r);
    }
    else {
...
void put(class rofftext  *self,int c)
{
    if (self->CurrentDiversion->SnarfOutput!=NULL)
        ...
        if (!self->v_TrapBlown && self->v_Trap) {
            ...
            cmd = (cmdfptr)(long)(self->Commands)->Lookup(self->v_Trap);
        ...
        if (self->HelpMode) {
            if (c == '\n')
                ...
                (self->textm)->AddInCharacter(self->pos++,'\n');
            }
        }

        if (c != NULCHAR)
	{
	    unsigned char w = self->Berliz[(unsigned char)c];
	    if (self->Fn) {
		(self->Fn)->AddInCharacter( self->fnpos++, w);
	    }
	    else {
		if (w == '\n') self->out_column = 0;
...
DEBUG(4, (stderr, "output %c at pos %d\n", w, self->pos));
		    (self->textm)->AddInCharacter( self->pos++, w);
		    self->out_column++;
		}
...
int get(class rofftext  *self,Trickle  t)
{
    ...
    if (c == self->EscChar) {
		...
		case 'h': {
		    ...
		    while (r-- > 0) s[i++] = ' ';
		    s[i] = '\0';
		    tpush(self, t, NULL, NULL, s, FALSE, 0, NULL);
		    c = get(self,t);
                    break;
		    }
        ...
        if (!translated && !self->v_RawMode) {
			...
			break;
                    ...
                    case '*':   /* stored string */
                        getname(self,t,temp);
                        tpush(self,t,NULL,NULL,getstring(self,temp),FALSE,0,NULL);
                        c = get(self,t);
                        break;
                    case 'n':   /* register output */
                        tpush(self,t,NULL,NULL,getregister(self,t),FALSE,0,NULL);
                        c = get(self,t);
                        break;
                    ...
                    case '$': /* argument */
                        temp[0] = get(self,t);
                        temp[1] = '\0';
                        tpush(self,t,NULL,NULL,self->CurrentContext->argv[atoi(temp)],FALSE,0,NULL);
                        c = get(self,t);
                        break;
...
#define CMD(name,fn) (self->Commands)->Store(Stringize(name),(char *)fn)
#define GOOFY(name,fn) (self->Goofy)->Store(Stringize(name),(char *)fn)
...
rofftext::rofftext()
{
    ...
    putregister(this,"dw",(int)tm->tm_wday+1,reg_Single,0,0);
    putregister(this,"dy",(int)tm->tm_mday,reg_Single,0,0);
    putregister(this,"mo",(int)tm->tm_mon+1,reg_Single,0,0);
    putregister(this,"yr",(int)tm->tm_year,reg_Single,0,0);
    putregister(this,"%",(int)1,reg_Single,0,0);
    putregister(this, ".h", 0, reg_Single, 0, 0);
    putregister(this, ".p", 10000, reg_Single, 0, 0);
    putregister(this, ".t", 10000, reg_Single, 0, 0);
    putregister(this, "nl", 1, reg_Single, 0, 0);
    putregister(this, ".k", 1, reg_Single, 0, 0);
    ...
    this->Goofy = new hash;
    this->ArgStack = new glist;
    this->SpecialChars = new hash;
    InitChars(this);    /* in roffchrs.c */
    CreateDefaultCommands(this);
    ...
    /* environment stack */
    CreateEnvirons(this);
    this->EnvironStack = new glist;
    this->DiversionStack = new glist;
    this->CurrentDiversion = CreateDiversion(this,NULL);
    ...
    for(i=0;i<256;i++)
        this->Berliz[i] = i;
    this->EscChar = '\\';
    this->Berliz[ESCAPE] = this->EscChar;

    this->v_InCond = 0;
    this->v_TrapBlown = FALSE;
    this->v_CopyMode = FALSE;
    this->v_TblMode = 0;
    this->v_TblTab = '\t';
    this->Tbl = NULL;
    this->baseline = 0;
    ...
    this->out_column = 0;
    this->tag_count = 0;
    this->v_ReadEscapes = TRUE;
    this->v_RawMode = FALSE;
    this->v_LastIfResult = TRUE;
    this->v_BOL = TRUE;
    this->v_NextBOL = TRUE;
    this->v_MultiSpace = FALSE;
    this->RoffType = FALSE;
    this->v_Trap = NULL;
    this->v_InputLineCount = 0;
    this->v_InputLineNumber = 0;
    this->v_ErrorCount = 0;
    this->v_TempIndentTrap = 0;
    this->v_BlankLine = TRUE;
    this->v_DiversionLevel = 0;
    this->current = '\n';
    for(i=0;i<8;i++) {
        this->ScaleFactor[i] = SCALE[i];
    }
    for(i=0;i<5;i++) {
        this->Fonts[i] = FONT[i];
    }
    ...
    (this)->ReadTemplate("roff",FALSE);
    this->textm = (class text *)this;

    DEBUG(1, (stderr, "stacks\n"));
    this->stack = (struct stackelt *)malloc(STACKSIZE*sizeof(struct stackelt));
    this->tempstack = (struct tempelt *)malloc(STACKSIZE*sizeof(struct tempelt));
    this->stack->env = (class environment *)(this->textm->rootEnvironment)->GetEnclosing( 0L);
    this->stack->pos = (this->stack->env)->Eval();
    this->stack->level = -1;
    this->tempstack->level = -1;
    this->styleID = 1;
    this->pos = 0;
...
static void DoCommand(class rofftext  *self,Trickle  t,char  *name,boolean  br)
{
    ...
    if ((cmd = (cmdfptr)(long)(self->Goofy)->Lookup(temp)) == NULL) {
        ...
        if ((self->Macros)->Lookup(temp) != NULL) {
            copy = TRUE; /* macro args read in copy mode */
        }
        /* have to look up command anyway */
        if ((cmd = (cmdfptr)(long)(self->Commands)->Lookup(temp)) == NULL) {
            ...
            switch (c) {
                ...
                case '\n':
                    ...
                    if (self->CurrentEnviron->NextInputTrap
                        ...
                        cmdfptr cmd2 = (cmdfptr)(long)(self->Commands)->Lookup( self->CurrentEnviron->InputTrapCmd);
...
long rofftext::Read(FILE  *file,long  id)
{
    ...
    if (! istroff(file)) return ReadFormatted(this, file, id);

    this->Input = topen(this,NULL,file,NULL);



    /* push input files in reverse order, so they will be read in sequence */

    if (this->inputfiles && *this->inputfiles) {
        for(begin = ptr = this->inputfiles;(*ptr != NULL);ptr++);
        for(ptr--;(ptr >= begin);ptr--) {
            if (strcmp(*ptr,"-")==0)
                tpush(this,this->Input,NULL,stdin,NULL,FALSE,0,NULL);
            else
                tpush(this,this->Input,*ptr,NULL,NULL,FALSE,0,NULL);
        }
    }

    if (this->macrofile) {
        tpush(this,this->Input,this->macrofile,NULL,NULL,FALSE,0,NULL);
    }

    /* scan until end of file */
    Scan(this,this->Input,NULL);
    ...
    if ((this->EndMacros)->Size()>0) {
        ...
        while(t = (struct trap *)(this->EndMacros)->Pop()) {
            ...
            if (cmd)
                (*cmd)(this,this->Input,0,1,this->Nullarg);
        }
    }
    Set_BOL(this);
...
boolean rofftext::InitializeClass()
{
#ifdef DEBUGGING
...
void rofftext::SetAttributes(struct attributes  *atts)
{
    ...
    while(atts!=NULL){
        ...
        if (strncmp(atts->key,"filename",8) == 0) {
            ...
            if (strcmp(key,"macrofile")==0) {
                ...
                this->PrintWarnings = atoi(atts->value.string);
            }
            else if (strcmp(key,"be-bogus") == 0) {
                this->BeCompletelyBogus = atoi(atts->value.string);
            }
        }
        atts=atts->next;
    }
...
long rofftext::ReadRoffIntoText(class text  *t,FILE  *fp,long  pos,char  **resources)
{
    ...
    long cc;
    class rofftext *r = new rofftext;

    (r)->SetInputFiles(resources);
    (r)->SetInitialPos(pos);
    (r)->SetText(t);
    cc = (r)->Read(fp,0l);
    (r)->Destroy();
...
void rofftext::SetText(class text  *t)
{
    (t)->ReadTemplate("roff",FALSE);
    this->stack->env =(class environment *) (t->rootEnvironment)->GetEnclosing( 0L);
    this->stack->pos = (this->stack->env)->Eval();
    this->textm = t;
}
