...
int main (int argc, const char **argv)
	...
	UTIME_SET(tvp, istat.st_atime, istat.st_mtime);
	if (UTIME(pnametmp, &tvp) < 0)
	...
	if(chmod(pname, mode)<0) {
	    printf("Couldn't chmod destination file %s\n", pname);
	}
	}
    /* all done now */
    exit(rcode);
