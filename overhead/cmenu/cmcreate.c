...
#ifndef NORCSID
#define NORCSID
static char rcsid[]="$Header: /afs/cs.cmu.edu/project/atk-src-C++/overhead/cmenu/RCS/cmcreate.c,v 2.35 1995/11/07 20:17:10 robr Stab74 $";
#endif
...
char *def_env;
{
	...
	if (windowAttributes.depth > 1 && CellsOfScreen(s) >= getprofileint("ForceMonochromeThreshold", 16)) {
	    ...
	    gcv.fill_style = /* FillTiled */ FillOpaqueStippled;
    else
	gcv.fill_style = FillOpaqueStippled;
...
char *def_env;
{
    ...
    if(getprofileswitch("MotifPopupMenusOnColor", TRUE)) {

	XWindowAttributes windowAttributes;
	if (XGetWindowAttributes(dpy, rootWindow, &windowAttributes)>0) {
	    if (windowAttributes.depth > 1 && CellsOfScreen(DefaultScreenOfDisplay(dpy)) >= getprofileint("ForceMonochromeThreshold", 16)) {
		motif_default = TRUE;
		iscolor = TRUE;	/* hint for below */
	    }
	}
    }
    if (getprofileswitch("MotifPopupMenus", motif_default))
	dp->motifMenus = TRUE;
