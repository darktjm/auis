...
#ifndef NORCSID
#define NORCSID
static UNUSED const char rcsid[]="$Header: /afs/cs.cmu.edu/project/atk-src-C++/overhead/mail/lib/RCS/authret.c,v 2.16 1994/06/09 18:17:42 rr2b Stab74 $";
#endif
	...
	if (ca != NULL) {
		...
		if (ca->UserName != NULL) {
			...
			for (S = ca->PersonName; *S != '\0'; ++S)
				if (strchr("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 !#$%&'*+-./=?^_`{|}~", *S) == NULL) break;
