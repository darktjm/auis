...
#ifndef NORCSID
#define NORCSID
static UNUSED const char rcsid[]="$Header: /afs/cs.cmu.edu/project/atk-src-C++/overhead/eli/lib/RCS/prims1.c,v 1.15 1994/06/09 18:07:48 rr2b Stab74 $";
#endif
...
EliSexp_t      *resbuf;
{
    ...
    cptr = strchr(EliStr_GetString(str1), *(EliStr_GetString(str2)));
    if (cptr) {
...
EliSexp_t      *resbuf;
{
    ...
    str2 = EliSexp_GetStr(restmp);
    cptr = strrchr(EliStr_GetString(str1), *(EliStr_GetString(str2)));
    if (cptr)
                ...
                if (selectval > 0) {
                    ...
                    if (readfdset & (1 << stdoutpipe[0])) {
                        ...
                        if (cc > 0) {
                            ...
                            memmove(stdoutstr + stdoutstrused, buffer, cc);
                            stdoutstrused += cc;
                        }
                        else {
                            if (cc < 0) {
                    ...
                    if (readfdset & (1 << stderrpipe[0])) {
                        ...
                        if (cc > 0) {
                            ...
                            memmove(stderrstr + stderrstrused, buffer, cc);
                            stderrstrused += cc;
                        }
                        else {
                            if (cc < 0) {
