...
void ATKELinkI::ProcessArgument(char *arg) {
	...
	if(*p && isspace(*p)) *p='\0';
	if(!in.eof()) {
	    if(strncmp(buf, "ATKListIni", 10)==0 || strncmp(buf, "ATKListFini", 11)==0) {
...
int ATKELinkI::ArgumentsDone() {
    ...
    out<<".align 4"<<endl;
    out<<".type ATK_IniList"<<",#object"<<endl;
    out<<"ATK_IniList"<<":"<<endl;
    ast->ResetIterator();
    int s=4;
    while(p=ast->NextSymbol()) {
	if(strncmp(p, "ATKListIni", 10)==0) {
	    out<<".word "<<p<<endl;
	    s+=4;
	}
    }
    out<<".long 0"<<endl;
    out<<".size ATK_IniList,"<<s<<endl;
    out<<".type ATK_FiniList"<<",#object"<<endl;
    out<<"ATK_FiniList"<<":"<<endl;
    ast->ResetIterator();
    s=4;
    while(p=ast->NextSymbol()) {
	if(strncmp(p, "ATKListFini", 11)==0) {
	    out<<".word "<<p<<endl;
	    s+=4;
	}
    }
    out<<".long 0"<<endl;
    out<<".size ATK_FiniList,"<<s<<endl;
    out.close();
    char cmd[MAXPATHLEN+1024];
    strcat(ofile2, ".o");
    sprintf(cmd, "as -K PIC -o %s %s", ofile2, afile2);
    if(system(cmd)!=0) {
