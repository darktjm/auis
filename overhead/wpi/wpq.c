...
{ /* If true, check the read/write (current, but less available) 
  ...
  if (FindCell(ThisDomain, &ca)) {
      /* We are probably not authenticated in our own cell.
       * Wing it and try ThisDomain as our cell.
       */
      WPCALL(wp_InitializeCell(ThisDomain, &wpdir));
      return;
  }
...
{ /* Find this user's Network ID, 
  ...
  if (FindCell(ThisDomain, &ca)) {
      /* Unauthenticated:  wing it using Unix uid. */
      return getuid();
  }
...
char *argv[];
{
  ...
  if (FindCell(ThisDomain, &ca)) {
      /* Unauthenticated:  wing it with ThisDomain as a guess for our cell name. */
      WPCALL(wp_InitializeCell(ThisDomain, &wpdir));
  } else {
      WPCALL(wp_InitializeCell(ca->CellName, &wpdir));
  }
