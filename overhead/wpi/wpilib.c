...
#ifdef CMU_ENV
    char *p;
    /* Don't allow bozos to forward to CMU.EDU, it causes mail loops */
    p = strrchr(out, '@');
    if (p) {
	lcstring(buf, p+1, sizeof(buf));
	if (strcmp(buf, "cmu.edu") == 0) {
	    strcpy(buf, "May not set forwarding address to CMU.EDU, could cause mail loop.");
	    error(buf, WPI_ERR_BADVALID);
	    return(uncool);
	}
    }
...
WPI_entry_t entry;
{
  ...
  if (p = strchr(Addr->LocalPart, '+')) *p = '\0';
...
WPI_entry_t entry;
{
  ...
  /* non-printable characters are always uncool */
  for (p=value; *p; p++) {
      if (!isascii(*p) || !isprint(*p)) {
	  error("Value contains non-printable characters",WPI_ERR_BADVALID);
	  return(uncool);
      }
  }
