...
#ifndef NORCSID
#define NORCSID
static UNUSED const char rcsid[]="$Header: /afs/cs.cmu.edu/project/atk-src-C++/ams/libs/ms/RCS/headers.c,v 2.13 1993/07/02 22:44:37 rr2b Stab74 $";
#endif
...
int MaxReturn, startbyte, *numbytes, *bytesleft;
{
	...
	*numbytes = 0;
    ...
    if (startbyte != Dir->CurPos && lseek(Dir->fd, startbyte, SEEK_SET) < 0) {
	errsave = errno;
	CloseMSDir(Dir, MD_READ);
