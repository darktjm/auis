...
#ifndef NORCSID
#define NORCSID
static char rcsid[]="$Header: /afs/cs.cmu.edu/project/atk-src-C++/ams/libs/ms/RCS/rawdb.c,v 2.70 1994/03/21 17:37:14 rr2b Stab74 $";
#endif
...
struct MS_Directory *Dir;
{
    ...
    if (Dir->CurPos != 0 && lseek(Dir->fd, 0, SEEK_SET) < 0) {
...
int             DoComplain;
{
    ...
    Dir->CurPos = AMS_DIRHEADSIZE;
    if (memcmp(DirHead, AMS_DIRECTORY_PREFIX_STRING, sizeof(AMS_DIRECTORY_PREFIX_STRING) - 1)) {
        if (DoComplain)
            AnnounceBadDirFormat(Dir->UNIXDir);
...
int            *msgnum;
{
	...
	if (i < SnapshotBufStart || i >= SnapshotBufStart + SnapshotBufLen) {
	    ...
	    if (Dir->CurPos != fpos && lseek(Dir->fd, fpos, SEEK_SET) < 0) {
	...
	if (i < SnapshotBufStart || i >= SnapshotBufStart + SnapshotBufLen) {
	    ...
	    if (lseek(Dir->fd, fpos, SEEK_SET) < 0) {
		AMS_RETURN_ERRCODE(errno, EIN_LSEEK, EVIA_GETSNAPSHOTBYID);
	    }
	    readct = read(Dir->fd, SnapshotBuf, sizeof(SnapshotBuf));
...
int             msgnum;
{
	...
	if (fpos != Dir->CurPos && lseek(Dir->fd, fpos, SEEK_SET) < 0) {
