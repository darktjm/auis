...
int *NumGood, *NumBad, *NumLocks, *resultcode, *FirstError, *NumInProgress, EliErrBufLim;
{
    ...
    *NumBad = *NumGood = *NumInProgress = *NumLocks = *FirstError = *resultcode = 0;
	...
	if (syscode==(-1)) {
	    ...
	    syscode = errno; /* the system() call failed, not the mail collection program */

	    sprintf(ErrorText, "Unable to execute personal mail collection command %s; error code was %d", ap_Shorten(PersonalMailCollectionCommand), syscode);
	...
	if (syscode==(-1)) {
	    ...
	    sprintf(ErrorText, "Unable to execute system mail collection command %s; error code was %d", ap_Shorten(AMS_MailCollectionCommand), syscode);
	    NonfatalBizarreError(ErrorText);
	    /* don't return */
	}
	else if (syscode) {
