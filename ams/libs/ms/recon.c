...
#ifndef NORCSID
#define NORCSID
static UNUSED const char rcsid[]="$Header: /afs/cs.cmu.edu/project/atk-src-C++/ams/libs/ms/RCS/recon.c,v 2.33 1995/06/25 22:15:02 rr2b Stab74 $";
#endif
    ...
    *NumGood = *NumBad = 0;
    ...
    if (oldDirReadable) {
        if (ReadOldMSDirectoryHead(oldMSDir)) {
            ...
            if (oldMSDir->CurPos != 0 && lseek(oldMSDir->fd, 0, SEEK_SET) < 0) {
                oldDirReadable = FALSE;
                CloseMSDir(oldMSDir, MD_APPEND);
            }
            else {
